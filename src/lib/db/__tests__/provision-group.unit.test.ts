...
describe("provisionGroupDb audit emissions", () => {
  afterEach(() => {
    vi.clearAllMocks();
    vi.restoreAllMocks();
  });

  it("emits group_db.provisioned on success with actorContext", async () => {
    ...
    const status = await provisionGroupDb(
      authPool as never,
      GROUP_ID,
  ...
  it("emits group_db.provision_retried on success with isRetry=true", async () => {
    const authPool = makeAuthPool();
    mockPoolPrototype();
    ...
    expect(mockAuditLog).toHaveBeenCalledWith(
      expect.objectContaining({
        action: "group_db.provision_retried",
        details: expect.objectContaining({ outcome: "active" }),
      }),
    );
  });

  it("emits group_db.provision_failed on failure with actorContext", async () => {
    const authPool = makeAuthPool();
    mockPoolPrototype();
    ...
    expect(mockAuditLog).toHaveBeenCalledWith(
      expect.objectContaining({
        action: "group_db.provision_failed",
        targetType: "group_db",
        targetId: GROUP_ID,
  ...
  it("emits group_db.provision_retried on failure with isRetry=true", async () => {
    const authPool = makeAuthPool();
    mockPoolPrototype();
