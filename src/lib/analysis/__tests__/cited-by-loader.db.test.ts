...
// real customer database with the ref-column GIN indexes applied. Validates
// that the JSONB containment (`@>`) probes match the exact persisted ref
// shapes (`{aice_id,event_key,generation,model_name,model,customer_id}` for
// events, `{story_id,...}` for stories), that superseded reports are
...
describe.skipIf(!hasPostgres)("reverse-citation lookup (db)", () => {
  ...
  beforeAll(async () => {
    ...
    // Persisted refs always carry `model_name`/`model` (#465) and
    // `customer_id` (#523); the containment probe pins the model, so every
    // seeded ref uses the full persisted shape.
    const eventRef = (
      aiceId: string,
      eventKey: string,
      generation: number,
    ...
    const storyRef = (storyId: string, generation: number) => ({
      story_id: storyId,
      generation,
      model_name: "openai",
      model: "gpt-4o",
    ...
    // #465 cross-model ref: a DEFAULT (gpt-4o) report that, under the
    ...
    await seedReport({
      ...
      eventRefs: [eventRef("aice-cov", "900", 1, "gpt-5.5")],
    });
  });
