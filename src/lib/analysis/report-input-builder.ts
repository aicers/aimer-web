...
export interface StoryRef {
   ...
   * report's own `subject_id`. Persisted refs always carry it; only the hash
   * canonicalization (`computeInputHash` strips the subject-default) and the
   * single-customer WIRE shape ({@link wireCustomerId}) omit it.
   */
  customer_id: string;
}
...
export interface ExemplarRef {
  ...
  model_name: string;
  model: string;
  /** Member subject id of the leaf's customer DB (#523), as on `StoryRef`. */
  customer_id: string;
}

/**
 * Resolve a WIRE source's member customer id (#524/#525). The single-customer
 * wire shape omits `customer_id` by design and resolves to the report's own
 * `subject_id` — every cited leaf lives in the subject's own customer DB —
 * while a group source carries the true member id. Symmetric with the
 * `computeInputHash` "omit `customer_id` when it equals the subject"
 * canonicalization. Wire shapes only: persisted refs always carry
 * `customer_id` and are read directly.
 */
export function wireCustomerId(
...
export function storyWireKey(
  ...
  return memberQualified
    ? `${wireCustomerId(ref, subjectId)}:${ref.story_id}`
    : ref.story_id;
}
...
export function eventWireKey(
  ...
  return memberQualified ? `${wireCustomerId(ref, subjectId)}:${base}` : base;
}
...
/**
 * Hash-side ref shape (#523): the canonical hashed form STRIPS `customer_id`
 * when it equals the subject, so the hash layer deliberately accepts refs
 * without it — looser than the persisted-ref contract, which always carries
 * it. A single-customer ref hashes byte-identically whether it carries
 * `customer_id == subject` or omits it.
 */
type HashRef<T> = Omit<T, "customer_id"> & { customer_id?: string };
...
function computeInputHash(args: {
  ...
  bucketDate: string;
  variant: ReportVariant;
  storyRefs: Array<HashRef<StoryRef>>;
  eventRefs: Array<HashRef<EventRef>>;
  exemplarRefs: Array<HashRef<ExemplarRef>>;
  aimerInputs: PeriodicReportInputs;
}): string {
