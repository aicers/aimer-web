...
export interface CitedStorySource {
   ...
   * member-customer detail, not the group subject. For a single-customer
   * report every persisted ref carries the report's own customer id, so the
   * link stays within the subject.
   */
  customerId: string;
  variant: CitedLeafVariant;
  display: {
...
// Stored `input_*_refs` shapes, mirroring the builder's persisted-ref types.
// `model_name`/`model` are the leaf's OWN model (#465), which under the
// never-drop fallback can differ from the report row's.
interface StoryRef {
  story_id: string;
  generation: number;
  model_name: string;
  ...
  // more than one, so de-redaction routes by it. On the single-customer path
  // it is the report's own subject id.
  customer_id: string;
}
interface EventRef {
  aice_id: string;
...
// Long-tail exemplar leaf ref (#495). There is no `lang` — exemplar leaves
// are always replayed at the English canonical language regardless of the
// row's `lang` / `restoration_lang`.
...
// column is untyped JSON and the native write path persists the parsed
// payload verbatim after only a top-level-object check (`report-worker.ts`) —
// `restoreUnits` / `restoreJoined` apply minimal per-section type guards.
...
export async function loadReportResultPage(
  ...
  // the single result pool (all refs carry `subjectId`); for a group, the
  ...
  const citedSources: CitedSources = {
    stories: storyRefs.map((ref, i) => {
      ...
      return {
        ...
        // The owning member customer id (#513) — the report's own subject on
        // the single-customer path.
...
function restoreReportSectionsFromRow(
  ...
  // `period_outlook` (a plain string) are NOT leaf-derived and carry no
  // citations: restore each entry and join into one display block. Neither
  // field's shape is validated by the write path (both sit outside
  // `LEAF_DERIVED_SECTION_KEYS`), so handle both shapes as a minimal type
  // guard over the untyped JSON.
  ...
  // `customer_id` and, for the citation, the wire source's optional
  // `customer_id` (the single-customer wire shape omits it, degrading to
  // `subjectId` via `refCustomerId`) — so on the single-customer path both
  // sides key by the subject.
  ...
  const decodeSource = (raw: unknown): CitedUnitSource | undefined => {
    ...
    if (wire.type === "story" && typeof wire.story_id === "string") {
      ...
      return {
        ...
        // degrading to `subjectId` for a single-customer source.
  ...
  // per-unit boundaries so a citation can anchor to one. The column is
  // untyped JSON and only the translate path enforces the array shape on
  // write, so a minimal `Array.isArray` guard yields no units for a
  // non-array section instead of throwing; non-unit items are skipped.
...
async function resolveReportCompareColumn(
  ...
  // carries `customerId` and routes back to `customerPool`.
 ...
 * owning member `customer_id` (the report's own subject on the
 * single-customer path). The leaf and its `event_redaction_map`
 ...
 * For a `customer` subject every ref carries the subject's own customer id
 * and `poolFor` returns the single result pool, so this collapses to one
 * batched leaf read plus one decrypt against that pool.
 *
...
async function buildReportTokenPlaintext(
  ...
  // The only shared pin across leaves: generation / model are per-ref.
  replayLang: string,
  ...
  for (const [cid, idxs] of groupByCustomer(exemplarCustomerIds)) {
    ...
    // English canonical `lang` (`ENGLISH_BASELINE`).
