...
export async function loadCitedByReports(
  ...
  // different generation of the same leaf must NOT appear here. The probe also
  // pins the leaf's `(model_name, model)` (#465 Scope 4): `generation` is
  // variant-scoped, so a model-less probe would re-attribute another model's
  // same-generation leaf citation. The probe deliberately omits `customer_id`:
  // `@>` is partial-object containment, so the match is member-agnostic — the
  // trail is already scoped to one customer DB via `subject_id`.
  const { column, probe }: { column: string; probe: string } =
