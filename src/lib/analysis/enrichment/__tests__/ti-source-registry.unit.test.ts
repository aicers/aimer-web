...
describe("derived SourcePolicy[] regression for the registered sources", () => {
  it("matches the previous inline LOCAL_FEED_POLICIES exactly", async () => {
    ...
    // the registry's stable-by-id sort, so the four Botvrij policies, the CERT
    // Polska policy, the ESET vendor-repo policy, the Infoblox policy, the MISP
    // warninglists negative source, and the three Phishing.Database policies
    // fall between the abuse.ch and Spamhaus groups.
    const { LOCAL_FEED_POLICIES } = await import("../local-feed-enricher");
