...
// Isolate the `domains:` hash from a target policy version. The segment is
// REQUIRED: `computePolicyVersion` (the only stamper) always emits it — an
// empty owned-domain set hashes to the `empty` sentinel, never an omitted
// segment — so a version without it is corrupt, and silently skipping the
// owned-domains drift check (a leak-prevention check) on such a version
// would mask the corruption. Treated like a corrupt range snapshot.
function targetDomainsFragment(targetPolicyVersion: string): string {
  const idx = targetPolicyVersion.indexOf("|domains:");
  if (idx === -1) {
    throw new SnapshotError("target_domains_missing");
  }
...
// leak. A target with no `|domains:` segment fails the job too
// (`targetDomainsFragment` throws): the segment is always stamped, so its
// absence means a corrupt version, not a skippable check.
