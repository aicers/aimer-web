...
//
// `GET /api/subjects/{subject_id}/analysis/story/{story_id}/summary`
//
  ...
  async (req: NextRequest, auth) => {
    ...
    const rows = await customerPool.query<{
      ...
      `SELECT priority_tier, severity_score, likelihood_score
        ...
        LIMIT 1`,
      [subjectId, storyId, DEFAULT_LANG, def.modelName, def.model],
    );
    ...
    return Response.json({
      ...
      score_kind: "leaf",
      link: `/subjects/${subjectId}/analysis/story/${storyId}`,
    });
  },
  { ctx: "general" },
);
