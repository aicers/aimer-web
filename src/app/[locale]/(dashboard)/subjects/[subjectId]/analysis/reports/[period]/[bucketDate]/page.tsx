...
export default async function ReportDetailPage({
  ...
  const { locale, subjectId, period, bucketDate } = await params;
  ...
  const basePath = subjectPages.report(locale, customerId, period, bucketDate);
  const switcher = (
  ...
  if (data.languageFallback) {
    ...
    const statusUrl = `${subjectApi.reportLanguageStatus(
      customerId,
      period,
      bucketDate,
    )}?${mergeQuery("", {
