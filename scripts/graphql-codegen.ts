...
const SDL_PATH = join(REPO_ROOT, "schemas/aimer.graphql");
...
//
// `StringNumber` carries aimer's `i128` values (e.g. nanosecond-resolution
...
function renderVariablesInterface(
  ...
  for (const v of variableDefs) {
    ...
    // variables object — emit it as `key?: T | null` so call sites
    // do not have to explicitly write `lang: null` when they want
    // to fall through to aimer's server-side default.
...
function emitInterface(def: InterfaceDef): string {
  ...
  const body = def.fields
    .map((f) => `  ${jsKey(f.name)}${f.optional ? "?" : ""}: ${f.tsType};`)
    .join("\n");
  return `export interface ${def.name} {\n${body}\n}\n`;
}
