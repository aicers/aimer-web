...
// `{ text, source? }` — uncited here, since the fixture pins empty input
// refs — while `baseline_observations` stays an array of Markdown strings
// and `period_outlook` a plain string. The text uses real Markdown
// (headings, lists, inline code) so the manual screenshots exercise the
// shared Markdown renderer (#382).
