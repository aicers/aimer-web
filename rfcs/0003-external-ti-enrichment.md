...
4. **Inject only redaction-token-aware facts into the LLM input** — the LLM never sees raw indicators, only token-referencing statements (e.g., `IP <<REDACTED_IP_001>> is on the abuse.ch Feodo C2 list`). The raw indicator and its match are used internally (steps 2–3) but are mapped back to the redaction token before anything reaches the model.
5. **Feed `known_ioc_hit`** into the existing `applyLikelihoodFloors`.

In short: **raw indicators are used internally for local matching and (opt-in) Tier 2 lookup; the LLM and any non-opted-in external service only ever see redaction tokens.**
