...
1. **Extract indicators from the raw (pre-redaction) member payload**, keeping the mapping `raw indicator ↔ redaction token`. Indicators must be extracted *before* redaction, or external malicious values may already be masked. Extraction is normalized (see "Indicator normalization").
2. **Match against enrichers**, dispatching each indicator to enrichers that `support()` its entity type. Tier 1 matches the **raw normalized indicator locally** (never leaves the host). Tier 2 may send the raw indicator off-host **only when the per-customer egress opt-in for that source is enabled**; every such egress is recorded in the per-customer egress audit log.
...
4. **Inject facts into the LLM input through the redaction policy** — every fact is run through the same redaction policy as event payloads before it reaches the LLM (RFC 0001 Amendment A). Per that policy, **customer-asset** indicators reach the LLM only as tokens (e.g., `<<REDACTED_DOMAIN_E1_001>> resolves to a host on the abuse.ch list`), while **external / pass-through** indicators (the usual enrichment target — attacker IPs outside any registered range, domains not matching a registered owned-domain) appear **raw**, identically to how they already appear raw in the redacted member text. The raw indicator and its match are used internally (steps 2–3); customer-asset values are mapped to their redaction token before anything reaches the model.
5. **Feed `known_ioc_hit`** into the existing `applyLikelihoodFloors`.

In short: **raw indicators are used internally for local matching and (opt-in) Tier 2 lookup; before reaching the LLM, facts pass through RFC 0001 Amendment A's redaction policy — customer-asset values become tokens, external indicators stay raw (consistent with the post-#422 payload policy). Any non-opted-in external service still never receives raw customer-asset values.**
