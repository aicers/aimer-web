...
### 4. In-context date toolbar (amendment — 2026-06-13)

Items 1–3 above are sound, but as first shipped they left a **discoverability gap**: the calendar lives only on a separate `/subjects/[id]/analysis/reports/[period]/calendar` page, reached from a faint underlined "view all" link on the report index. The date control is therefore **absent at the two moments a user actually forms the intent to pick a date** — (a) landing on a subject's report index ("show me a date other than the recent ones listed"), and (b) reading a report ("I'm on 06-12, take me to 06-10 / last week"). Navigating reports feels indirect because the tool is not where the intent is.

**Decision: a single, consistent date toolbar on both the report index and the report detail surfaces.** The toolbar carries the period tabs (existing), the within-period prev/next of item 2, and a calendar button:

```
[ LIVE | DAILY | WEEKLY | MONTHLY ]    ◀  2026-06-12  ▶    [📅 calendar]
```

- **Calendar opens as an anchored popover** (decided), rendering the period-matched grid (item 1) in place; selecting a bucket jumps to its report. It shows **one period's grid, on demand, collapsed by default** — so it **preserves the §1.4 intent** ("the hub does not render a full calendar inline") rather than overturning it: this is an explicit, narrow exception (a single-period on-demand popover), not four always-expanded grids on the hub. A centered modal was considered and rejected in favor of the lighter anchored popover.
- **The toolbar is the same control in both places.** No mode switch between index and detail; the user never has to wonder where the calendar went.
- **The standalone `/…/calendar` page is retained** as a deep-link / shareable / no-JS-fallback target. The popover is the in-context everyday path; the page is the addressable one.

**Data approach (decided — reuse the loader, not a new authz surface).** The popover fetches each viewport (month/year) client-side via a **thin route handler that calls the existing `loadReportCalendarPage` loader and reuses its status mapping** (unauthorized → 404, forbidden → 403). Authorization (subject-kind resolution, group DB, retention boundary, existence-hiding) is **not reimplemented** — the loader stays the single authz site; the handler only re-exposes it. *Rejected: a Next.js intercepting-route modal* — intercepting from both the index and the detail (different route depths) is finicky, yields a centered modal rather than an anchored popover, and is hard to specify deterministically.

**Top-level "보고서" menu (decided — option 1).** The cross-customer `/reports` overview has **no single subject**, so it carries **no date toolbar**; it stays a "most-recent / highest-risk + subject entry point" surface, consistent with the subject-first navigation above. The toolbar applies **from the subject level down** (index + detail). Restructuring the top-level menu to jump straight into a subject's dated view is a **separate, later concern**, not part of this work.

**Scope.** This is **one self-contained implementation PR on the subject surfaces only**: the toolbar on index + detail, the anchored calendar popover, the thin loader-backed endpoint, tests, and the EN/KR manual update. The top-level-menu treatment is explicitly out of scope (its own issue once the IA question is decided).
...
- [ ] In-context date toolbar (§Temporal navigation item 4) — a consistent period-tabs + prev/next + calendar-button toolbar on both the subject report **index** and **detail**, with the calendar as an **anchored popover** (period-matched grid, one period on demand) backed by a thin route handler that reuses `loadReportCalendarPage` + its status mapping (no reimplemented authz). Retain the standalone `/…/calendar` page as a deep-link target. Subject surfaces only — the cross-customer `/reports` overview and any top-level-menu restructure are out of scope. Includes tests + EN/KR manual.
