...
Baseline events enter periodic reports as aggregated statistics, **and** (amendment, #489) loose individual baseline events — those not correlated into a story — may also be auto-analyzed by a cost-bounded background worker: tier A (deterministic TI/IOC hit, guaranteed) and tier B (best-effort under a configurable per-customer daily cap). Their analyses become individual event leaves (citation + aggregates + long-tail narrative); story members are excluded. See §"Individual baseline-event auto-analysis (amendment, #489)".
...
- aimer already supports `generateReport(DAILY/WEEKLY/MONTHLY)`, but it builds reports purely from previously-analyzed single events. Phase 2 ingestion (baseline, story) does not pass through single-event analysis (cost-prohibitive), so today's `generateReport` has nothing to summarize on the Phase 2 side. (amendment, #489: individual baseline events can now be auto-analyzed under a configurable cost cap rather than excluded outright — see §"Individual baseline-event auto-analysis".)
...
| Story detail page | "AI narrative analysis" badge with priority tier | `/subjects/{subject_id}/analysis/story/{story_id}` (canonical; `/customers/{customer_id}/analysis/story/{story_id}` kept as an inbound alias — see RFC 0004 / #503) |
...
- **Allowed**: redacted `story_member.event` JSONB, redacted `detection_events.redacted_event` JSONB, baseline **aggregate statistics** (counts, distributions, deltas — derived from `baseline_event` columns but stripped of raw payloads), and (amendment, #489) the redacted `baseline_event.raw_event` JSONB for individual baseline-event analysis — already redacted at ingest under its `redaction_policy_version` and keyed in the `(aice_id, event_key)` map, same stance as `detection_events.redacted_event`.
...
- **Disallowed**: `story.summary_payload` raw content, `policy_run.summary_stats` raw content, `baseline_event.window_signals`, any **unredacted** upstream event payload, any other column not covered by the event-level redaction map. (amendment, #489: the *redacted* `baseline_event.raw_event` moved to Allowed above for individual baseline-event analysis; the raw upstream payload is still never stored or sent — ingest redacts before storage.)
...
POST /api/subjects/{subject_id}/analysis/report/{period}/{bucket_date}/regenerate
        ?tz=...&lang=...&model_name=...&model=...      (all optional)
```

(Canonical `/api/subjects/...` since RFC 0004 / #503; `/api/customers/...`
is the inbound-compatibility page alias only — for a customer
`subject_id == customer_id`. The `/api/admin/customers/[customerId]/...`
admin layer is unaffected.)
...
3. Select top single-event analyses *excluding* anything in the set. Event leaves include both manually analyzed `detection_events` and, per the amendment (#489), auto-analyzed loose `baseline_event` rows (those carrying an `event_analysis_result`); both dedup against the story set identically. Citation uses the tier-guaranteed + hard-ceiling policy; the uncited remainder feeds the long-tail narrative and the full-set aggregates (see §"Individual baseline-event auto-analysis").
...
This amendment reverses the original decision (§Summary; Decision log 2026-05-25) that baseline events are never analyzed individually. That exclusion was made on cost grounds; this amendment keeps the cost ceiling but makes it **explicit and configurable** rather than implicit-zero, so that loose high-interest baseline events — ones aice-web-next did not correlate into a story and no analyst sent for ad-hoc analysis — can become individual report leaves instead of being invisible outside the aggregates.

### Selection principle

A selection criterion that aice-web-next could compute itself does not belong in aimer-web (it would simply send or withhold the event upstream). aimer-web therefore does not re-filter baseline events on sender-owned signals (`raw_score`, `kind`, `category`, `window_signals`, `scoring_weights_snapshot`). The default posture is to analyze **all** non-story baseline events; the only selective judgment is load-shedding under a cost cap, prioritized by an aimer-only signal (the deterministic TI/IOC hit, which aice-web-next cannot produce because it has no access to the threat-intel feeds).

### Two tiers and a configurable daily cap

For each ingested `baseline_event` that is not a member of any story (story members are covered by story-level analysis and deduped out of event leaves — see §"Dedup across Phase 1 and Phase 2"):

- **Tier A — guaranteed, uncapped.** An event with a floor-eligible deterministic TI/IOC hit (RFC 0003 enrichment, extended to individual events — see below) is always queued for analysis, regardless of the cap.
- **Tier B — best-effort, capped.** Every other loose baseline event is a candidate, analyzed up to a configurable per-customer daily count cap. Once the day's tier-B budget is exhausted, remaining tier-B events are skipped (recorded with a terminal budget-skipped marker, not deferred into later days). The budget resets at the customer-timezone calendar-day boundary, aligned with report windows. Within a day, candidates are processed in neutral `event_time` / `received_at` order — no sender-field re-ranking, per the selection principle.

IOC-vs-cap: the cap governs tier B only; tier-A events are always analyzed on top, so the daily total is `#IOC_hits + min(cap, tier-B volume)` — the cap is a tier-B budget, not an absolute ceiling. Cap resolution mirrors the #473 default-model three-tier order: per-customer override → admin-set global default → env fallback. A cap of zero disables tier B (only tier-A IOC events are analyzed), which is a valid tuning point while real volume is unknown (pre-release). Because tier A is uncapped, total cost is **not** absolutely bounded — an IOC spike or a noisy/false-positive feed can drive `#IOC_hits` up — so operational guardrails are required and tracked as implementation follow-up: monitoring/alerting on the auto-analysis rate and an emergency disable (cap=0 plus a tier-A kill switch).

### Enrichment dependency (RFC 0003)

Tier A needs a per-individual-baseline-event IOC verdict. RFC 0003 enrichment is today story-scoped (`story.known_ioc_hit`, `story_ioc_evidence`, `story_enrichment_fact`); the per-event IOC-matching machinery inside the enrichment worker is reused/extended to enrich standalone baseline events, producing a per-`(source_aice_id, event_key)` verdict. Tracked as an RFC 0003 change.

### Infrastructure

Mirrors the story-analysis pipeline: a new `event_analysis_job` table (grain `(customer_id, aice_id, event_key, lang, model_name, model)`; `queued`/`processing`/`done`/`failed`, generation, attempts, backoff, dry_run, force columns), seeding extended into `applyBaselineIngestHook` (tier-A unconditional, tier-B budget-gated), a new worker tick (`FOR UPDATE SKIP LOCKED`, advisory lock per event, backoff, stuck recovery), and a generalized analyze primitive fed from the **stored redacted** `baseline_event.raw_event` payload — already redacted at ingest under the row's `redaction_policy_version` and keyed in the `(aice_id, event_key)` map (the same stance as `detection_events.redacted_event`), so no raw upstream payload reaches the LLM and the stored policy version is reused so redaction stays constant rather than recomputed — instead of coupling to `detection_events`; the story worker's stored-payload analysis is the template. Model resolves via `resolveDefaultModel(customer_id)` (#473). Results reuse `event_analysis_result`, with an `origin` discriminator (`manual` | `auto_baseline`) and a system principal (or nullable `requested_by`) for the non-human requester. This requires a schema change to `event_analysis_result` — an RFC 0001 storage-model table — adding the `origin` column and relaxing/handling the `requested_by NOT NULL` constraint; the implementation must carry that RFC 0001 / DDL change explicitly. Because a baseline event is atomic (no members arriving over time), the heavy story `pending → ready → dirty` settle machine is unnecessary; a lighter enqueue-on-ingest + budget-gated seed model is used.

### Report integration

Auto-analyzed events appear through the existing `eventAnalyses` leaf path — `selectTopEvents` already joins `event_analysis_result` to `baseline_event` — so no new query path is needed for citation. The larger analyzed pool turns three things into deliberate choices:

- **Citation (`notable_events`)**: tier-guaranteed with a hard ceiling M — every CRITICAL/HIGH leaf is individually cited up to M; overflow and lower tiers fall to the long-tail narrative; ranking is unchanged (tier, then `severity + likelihood`).
- **Aggregates**: the technique leaderboard / tier distribution / factor weaving range over the **full analyzed-in-window set**, not only cited leaves (`baseline_event` carries no TTP tags, so analyzed-leaf `ttp_tags` are the only in-window technique source — now far richer).
- **Long-tail narrative**: the uncited remainder is characterized as synthesized prose (dominant technique clusters, severity makeup, asset concentration, notable repetition), not dropped and not bare counts. This keeps tier-B analysis valuable even on days the cap binds.

The round-14 coverage/calibration split is unchanged: calibrated scores come from the report-model subset; because auto-analysis uses the per-customer default model (#473), default reports and auto-leaves calibrate together.

### aimer contract (cross-repo)

The long-tail narrative requires the long tail to reach the prompt as a dedicated input section. `generatePeriodicSecurityReport`'s `PeriodicReportInputs` gains an `analyzedEventAggregates` section (window bounds, `analyzedCount`, `citedCount`, full-set `topTechniques`, `tierDistribution`, `uncitedRollup`, and `exemplars` carrying representative report-scope redacted factor phrases), with a `PERIODIC_SECURITY_REPORT_PROMPT` revision and `promptVersion` bump. Because the aimer SDL is vendored byte-for-byte (`docs/SCHEMAS.md`), the aimer Rust change lands first, then the aimer-web SDL refresh + builder. Exemplars carry report-scope redacted tokens only — same redaction stance as cited leaves; no raw payload reaches the prompt.

### Scope

This RFC text covers design only. Implementation is decomposed into follow-up issues after this amendment merges and spans three surfaces: aimer (Rust contract + prompt), RFC 0003 (per-event enrichment), and aimer-web (worker, tables, cap config, report builder, SDL refresh).
...
- 2026-06-09 (amendment, RFC 0004 / #503): the analysis surface (pages **and** API) moves from `/customers/{customer_id}/...` to **`/subjects/{subject_id}/...`**, which is now the **canonical** path; `/customers/{customer_id}/...` is retained as an **inbound-compatibility alias only** (a locale-preserving redirect — `src/proxy.ts`), so the aice-web-next deep-link contract above keeps resolving unchanged. RFC 0004 introduces a `subject` supertype that customers (and, later, groups) extend: a customer is a `kind='customer'` subject sharing its UUID, so `subject_id == customer_id` for every customer and the move is a pure path swap with no id translation. The deep-link table entry at §"What aice-web-next surfaces" and the §"Force regenerate" endpoint URLs are restated with `/subjects/{subject_id}` canonical (the `/api/admin/customers/[customerId]/...` admin layer is unaffected and stays customer-scoped). The periodic-report storage (`periodic_report_state` / `periodic_report_job` / `periodic_report_result`) is re-keyed from `customer_id` to `subject_id` so group-produced periodic reports share the same tables; `story_analysis_*` / `event_analysis_*` stay `customer_id`-keyed (customer-side analysis orchestration). No analysis behavior or bucket semantics change. This is bookkeeping for the RFC 0002 contract surface; the full design and decomposition live in RFC 0004 (#502).
- 2026-06-08 (amendment, #489): reverses the 2026-05-25 "baseline events never analyzed individually" decision. Loose baseline events (not story members) may now be auto-analyzed by a cost-bounded background worker — **tier A** (deterministic TI/IOC hit via RFC 0003 enrichment extended to individual events; guaranteed, uncapped) and **tier B** (all other loose events, best-effort under a configurable per-customer daily count cap with customer-tz reset, neutral time-order, terminal budget-skipped marker on overflow). Selection principle: aimer-web does not re-filter on sender-owned signals (`raw_score` etc.) — the default is analyze-all, and the only selective judgment is load-shedding prioritized by the aimer-only IOC signal. IOC is outside the cap (cap is a tier-B budget, not an absolute ceiling); cap resolution mirrors #473 (per-customer → admin-global → env). Infrastructure mirrors the story pipeline (`event_analysis_job`, ingest-hook seeding, worker tick, analyze primitive generalized to read the stored redacted `baseline_event.raw_event` (redacted at ingest under its stored policy version), `event_analysis_result` reuse with an `origin` discriminator, default-model resolution per #473); a lighter enqueue-on-ingest model replaces the story settle machine since baseline events are atomic. Report integration: auto-leaves flow through the existing `eventAnalyses` path; citation is tier-guaranteed + hard-ceiling M; aggregates broaden to the full analyzed-in-window set; the uncited remainder gets a synthesized long-tail narrative. The narrative requires an aimer contract extension (cross-repo): `PeriodicReportInputs` gains an `analyzedEventAggregates` section (+ `exemplars`) with a prompt revision and `promptVersion` bump, the aimer Rust change landing before the aimer-web SDL refresh. §Summary, §Motivation, §"Dedup across Phase 1 and Phase 2", and §"Redaction stance" (the redacted `baseline_event.raw_event` moved Disallowed → Allowed for individual analysis) edited accordingly; new §"Individual baseline-event auto-analysis" added. The `event_analysis_result` reuse needs an RFC 0001 / DDL change (`origin` column + `requested_by` handling), and tier A's uncapped cost requires operational guardrails (rate alert + emergency disable) — both carried by the implementation. Implementation is decomposed in follow-up issues after this amendment merges and spans aimer + RFC 0003 + aimer-web.
