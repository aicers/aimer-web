...
- **(b) Deterministic acceptance suite** (preferred in pre-release dev where (a)'s wall-clock is the dominant constraint): a fixture-driven test exercises the eight transition scenarios listed below. CI-green on the suite plus manual review of one fixture run is the equivalent passing criterion. The worker reads `NOW()` through a mockable seam (e.g., `getCurrentTimestamp()` helper) so timers (story idle 15min, story max-wait 6h, DAILY settle 3h, idle-quiet 30min) can be fast-forwarded deterministically without wall-clock.

The eight transition scenarios the suite must cover (each verified by SQL inspection of state row history + emitted job rows):
...
5. **DAILY settle → ready** — fast-forward bucket end + `ANALYSIS_SETTLE_HOURS_DAILY` + 30min idle-quiet → ready (additional case: 1h settle when a strict `cursor_watermark` covers the bucket).
6. **Archived → re-pending cycle** — window-replace removes all versions of a `story_id` → state `archived`; same `story_id` re-appears → state row reset to `pending` and prior dry-run job rows deleted (#294 decision 1).
7. **Reconciliation seed** — delete one state row after ingest, run reconciliation tick → row re-seeded; second tick reports zero changes (#294 decision 2).
8. **Generation cap exemption (dry-run)** — repeat dirty trigger past `ANALYSIS_MAX_GENERATION` on a dry-run job → cap not applied to dry-run rows, real-call jobs would still cap.

The synthetic suite is tracked in a dedicated aimer-web acceptance-suite issue (filed as the round-12 follow-up; see #294 body's "Verification gate" section for the link). The 48h path remains a valid alternative.
...
- 2026-05-27 (review round 12): Phase 0 verification gate redefined to accept either (a) real-environment 48h observation (unchanged) or (b) a deterministic acceptance suite covering the eight documented state-transition scenarios (story idle, story max-wait, story dirty cascade, periodic refresh-window overlap, DAILY settle, archived → re-pending cycle, reconciliation seed cleanliness, generation cap dry-run exemption). Worker reads `NOW()` through a mockable seam so timers can be fast-forwarded deterministically. CI-green on the suite + manual review of one fixture run is equivalent to passing the 48h observation. Rationale: 48h's wall-clock cost was the dominant constraint in pre-release dev, and the verification value the 48h captured was an exhaustive run through documented transitions under timer firing, refresh-window cascade, and reconciliation second-pass — all reproducible deterministically once the time seam exists. Path (a) remains valid. Phase 1/2/3 gates are unaffected; they include qualitative content (operator review, near-duplicate detection, comparative framing) that a fixture cannot substitute. New aimer-web acceptance-suite issue tracked as the round-12 follow-up implementation.
