...
- **(b) AI-proxy review** (preferred when operator time is constrained): produces `docs/verification/phase-2-gate-{YYYY-MM-DD}.md` covering:
  1. **Automated metrics**: day-over-day `analysis_text` diff ratio (>40% similarity flags near-duplicate); aggregate score variance per period; baseline-drift signal-to-noise (proportion of days drift materially shaped `priority_tier`); leaf-reference cardinality per report.
  2. **Narrative scoring on 5 daily reports + 2 LIVE snapshots**, each scored on (i) section-structure adherence (`executive_summary` / `story_highlights` / `baseline_drift` / `notable_events` / `recommendations` present and distinct), (ii) leaf-reference accuracy (cited stories actually map to included `input_story_refs`), (iii) TTP weaving (techniques referenced by ID in narrative match `aggregate_ttp_tags`), (iv) day-over-day distinctness (paraphrase rather than re-summarize is flagged).
...
If proxy and human spot-check agree on ≥ 90% of flagged items across two consecutive gate passes via path (b), the next phase's path (b) may proceed without a full spot-check — the human just stamps the proxy report's gate-pass recommendation. Conversely, if any single gate pass shows < 70% proxy/human agreement, the **next** gate reverts to path (a) regardless of operator availability. Agreement metric is recorded in each `docs/verification/phase-{N}-gate-{date}.md` artifact (proxy-flagged count, human-confirmed count, human-overridden count) so the trail is auditable.
