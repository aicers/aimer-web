...
**Step 2 — rename at prompt-build (read only, never re-redact).** RFC 0002 (§"multi-event token namespacing") assembles a story prompt by renaming each event's per-event tokens `<<REDACTED_{TYPE}_{NNN}>>` to **story-scope** `<<REDACTED_{TYPE}_E{i}_{NNN}>>`, where `i` indexes the event in `input_event_refs`. Fact tokens are reconciled in the **same rename pass** — no value is re-examined, only tokens are renamed. Because payload and fact share one write-time policy, a customer-asset indicator that appears in a member event is already tokenized in that event's map (for **both** IPs and domains), so the fact's token can be reconciled to the member's. For each indicator carried by a fact:
