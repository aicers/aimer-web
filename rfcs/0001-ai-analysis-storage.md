...
**Storage home — story-agnostic only.** The cache persists, per stored fact, exactly what is reusable across stories: the **redacted fact text** and the **encrypted self-scoped fact map** (`fact_token → { kind, value }`), plus the fact's **`redaction_policy_version`**. Introduce an `enrichment_redaction_map` table (or equivalent columns on the enrichment cache/result row) in the same shape as `event_redaction_map` — encrypted via the same DEK/KEK envelope, keyed to the enrichment row rather than `(aice_id, event_key)`. It stores **no story- or member-specific data** (no `(aice_id, event_key)`, no `E{i}`/`F{k}`): those are not properties of the indicator, they are properties of a particular story's prompt, so putting them on the cross-story cache would break reuse. Without this home the DB-write-boundary rule cannot be implemented or demapped; it is part of this amendment's scope (see the sub-issue breakdown).
...
Reconciliation is per-story because the correct answer is per-story: the same cached fact may be used in story X (which contains the member event) and story Y (which does not), and "the member with the lowest `E{i}`" differs by story. So at build the worker — which is **already decrypting this story's member maps to assemble the prompt** — also has the fact's self-scoped map (decrypted the same way). For each customer-asset `fact_token`, it looks for a member map entry in *this story* with the **same `{ kind, value }`**:
...
- No match in this story → the indicator is reflection-only **for this story**, so it takes a fact-scope `<<REDACTED_{TYPE}_F{k}_{NNN}>>`, with its value carried from the fact's self-scoped map into this story's render-time demap state.

This value comparison is in-memory at build, over maps the builder already decrypted for RFC 0002's own rewrite — it adds no new plaintext exposure and no on-disk cross-reference. It also leaves the **`event_redaction_map` entry shape unchanged**. (An implementation may *cache the per-story reconciliation result* on the story result row to avoid recomputing on re-render — but that derived link is keyed to the story, not to the cross-story enrichment cache. A content-addressed variant — storing `normalizedIndicatorHmac` on both maps and matching on that instead of `{ kind, value }` — is an allowed optimization but not required; the normative rule is "match this story's member maps at build.")
