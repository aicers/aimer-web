...
- **External / pass-through indicators** (attacker IPs outside any registered range, domains not matching a registered owned-domain) are **not** redacted — in the member payload or in a fact. They reach the LLM **raw**, identically in both. This is the post-#422 default and is exactly the threat intelligence the operator needs.
- **Customer-asset indicators** (an IP in a registered range, a domain matching a registered owned-domain) **are** redacted to a token. Here two concerns arise:
  1. **Token consistency.** If the same customer indicator also appears in a member event, the fact must reach the LLM as the **same token** that member text already uses, or the model cannot connect the fact to the event. A.1 defines how.
  2. **Reflection leak.** TI responses can *reflect* customer assets back — passive-DNS neighbors, co-resolving hosts — that never appeared in any member payload and so were never tokenized. A fact carrying `vpn.customer.example` (a registered owned-domain) must be redacted even though no member event mentions it. A.1's fact-scope handles this case.

The redaction decision is identical to event-payload redaction; what is **new** here is only (a) a domain KIND so owned-domains can be matched at all (A.2), and (b) a fact-scope for customer-asset indicators that appear *only* in a fact (A.1). This applies to facts from **every source** — curated feed, online API, and the LLM/web-search soft-only fallback generator in RFC 0003. Therefore:

> **Every enrichment fact is run through the same redaction policy as event payloads before reaching the LLM, regardless of source. External indicators pass through raw; customer-asset indicators are tokenized.**

### A.1 — Fact indicator handling: pass-through vs. token
