...
```json
{
  "<<REDACTED_IP_001>>": { "kind": "ip", "value": "203.0.113.5", "hmac": "…", "hmacKeyVersion": "k1", "normalizationVersion": "n1" },
  "<<REDACTED_EMAIL_001>>": { "kind": "email", "value": "alice@example.com", "hmac": "…", "hmacKeyVersion": "k1", "normalizationVersion": "n1" }
}
```

Serialised JSON, then encrypted via the existing DEK/KEK envelope (`src/lib/crypto/envelope.ts`) before storage.

The `hmac` / `hmacKeyVersion` / `normalizationVersion` fields are **added by Amendment A** (and apply to both `event_redaction_map` and the new `enrichment_redaction_map`). `hmac` is the keyed HMAC of the normalized indicator value — the same construction RFC 0003 uses for evidence records — and lets the prompt-build pass match a fact token to a member token **without decrypting either value to plaintext** (see Amendment A.1, step 2). Pre-Amendment-A entries were `{ kind, value }`; the addition needs no migration (engine pre-release, per Amendment A.4).
...
- `ai_analysis.ttp_tag_dropped` — at least one LLM-returned `ttp_tags` ID was filtered before storage. Payload: target `(customer_id, aice_id, event_key)` for event-level, `(customer_id, story_id)` for story-level (set whichever applies; the other is NULL); `dropped_ids: string[]`; `reason: 'not_in_vendored_mitre' | 'invalid_format'`; `mitre_vendor_version` snapshot. Emitted once per `(row, reason)` group — IDs dropped for the same `reason` within a single write coalesce into one row; mixed-reason drops produce one row per distinct `reason`.
- `ai_analysis.factor_dropped` — at least one item in `severity_factors` or `likelihood_factors` was filtered before storage. Payload: target identifiers as above; `axis: 'severity' | 'likelihood'`; `dropped_items: string[]`; `reason: 'sentence_start' | 'oversized' | 'empty' | 'all_items_filtered'`; `replaced_with_sentinel: boolean` — `true` when post-filter recovery substituted `["insufficient evidence"]`. Emitted once per `(row, axis, reason)` group. When sentinel recovery substitutes `["insufficient evidence"]`, the recovery is reported on a row with `reason: 'all_items_filtered'` and `replaced_with_sentinel: true`; that row may coexist with per-reason rows for the same `(row, axis)` describing the underlying shape violations. Cap-only trim (RFC 0002:725) does not emit — RFC's `reason` enum has no cap value by design.
...
Two concerns then arise at prompt-build (a **read + rename** step, never a re-redaction):

1. **Token consistency.** If a customer indicator also appears in a member event, the fact's token must be renamed to the **same story-scope token** the member text uses, or the model cannot connect the fact to the event. A.1 defines the reconciliation.
2. **Reflection leak.** TI responses can *reflect* customer assets back — passive-DNS neighbors, co-resolving hosts — that never appeared in any member payload. A fact carrying `vpn.customer.example` (a registered owned-domain) is redacted at write time like any other customer asset; at prompt-build it has no member event to anchor to, so it takes a fact-scope token (A.1).

The redaction decision and timing are identical to event-payload redaction; what is **new** here is only (a) a domain KIND so owned-domains can be matched at all (A.2), and (b) a fact-scope rename for customer-asset indicators that appear *only* in a fact (A.1). This applies to facts from **every source** — curated feed, online API, and the LLM/web-search soft-only fallback generator in RFC 0003. Therefore:
...
### A.1 — Fact storage redaction, and prompt-build token rename
...
**Storage home — story-agnostic only.** The cache persists, per stored fact, exactly what is reusable across stories: the **redacted fact text** and the **encrypted self-scoped fact map** (`fact_token → { kind, value, hmac, hmacKeyVersion, normalizationVersion }`), plus the fact's **`redaction_policy_version`**. Introduce an `enrichment_redaction_map` table (or equivalent columns on the enrichment cache/result row) in the same shape as `event_redaction_map` — encrypted via the same DEK/KEK envelope, keyed to the enrichment row rather than `(aice_id, event_key)`. It stores **no story- or member-specific data** (no `(aice_id, event_key)`, no `E{i}`/`F{k}`): those are not properties of the indicator, they are properties of a particular story's prompt, so putting them on the cross-story cache would break reuse. Without this home the DB-write-boundary rule cannot be implemented or demapped; it is part of this amendment's scope (see the sub-issue breakdown).

**Step 2 — reconcile + rename at prompt-build (per story; read only, never re-redact).** RFC 0002 (§"multi-event token namespacing") assembles a story prompt by renaming each event's per-event tokens `<<REDACTED_{TYPE}_{NNN}>>` to **story-scope** `<<REDACTED_{TYPE}_E{i}_{NNN}>>`, where `i` indexes the event in `input_event_refs`. Fact tokens are reconciled in the **same pass, against *this* story's members** — never via a link baked into the cache. "Read only, never re-redact" means no *raw value is re-scanned or re-classified*; reconciliation only matches already-tokenized values.

Reconciliation is per-story because the correct answer is per-story: the same cached fact may be used in story X (which contains the member event) and story Y (which does not), and "the member with the lowest `E{i}`" differs by story.

It is done **without decrypting any value to plaintext**, by matching on the keyed **`hmac`** carried on every map entry (both the member maps' and the fact map's, per the map shape above). RFC 0002's prompt-build pass today only rewrites token *strings* and does not open maps (it decrypts only later, at render/audit time); this amendment **does not add a plaintext decryption step to prompt-build** — it reads the `hmac` field, which is itself non-reversible. For each customer-asset `fact_token`, the builder looks for a member map entry in *this story* with the **same `hmac` (and `hmacKeyVersion`/`normalizationVersion`)**:

- Match found → rename the `fact_token` to that member's story-scope `<<REDACTED_{TYPE}_E{i}_{NNN}>>` (lowest `i` among matching members in *this* story's `input_event_refs`).
- No match in this story → the indicator is reflection-only **for this story**, so it takes a fact-scope `<<REDACTED_{TYPE}_F{k}_{NNN}>>`. Only in this branch is the value needed, and only later — at render/demap time, resolved from the fact's self-scoped map exactly as RFC 0002 resolves `E{i}` values from per-event maps. The reconciliation itself never reads a value.

Matching on the HMAC (not plaintext) is the reason Amendment A adds `hmac` to the map entry shape: it keeps prompt-build's existing "no plaintext at build" trust boundary intact. The alternative — decrypting member maps at build to compare `{ kind, value }` — was rejected precisely because it would extend plaintext exposure into the prompt-build path, which RFC 0002 deliberately keeps value-free. An implementation may additionally *cache the per-story reconciliation result* on the story result row to avoid recomputing on re-render; that derived link is keyed to the story, never to the cross-story enrichment cache.

Because payload and fact share one write-time policy, a customer-asset indicator that appears in a member event is tokenized in that event's map (for **both** IPs and domains), so the per-story match always finds it when the member is in this story. For each indicator carried by a fact:

- **External / pass-through indicator** (stored raw — attacker IP outside every range, domain matching no owned-domain). Emit it **raw**; there is no token and none is needed. This is the common case for enrichment (the indicator the fact is *about* is usually external). An earlier draft wrongly assumed every fact indicator had a token to reuse.
- **Customer-asset indicator present in exactly one member event `i`.** It is tokenized in that event's map, so rename the fact's token to the story-scope `<<REDACTED_{TYPE}_E{i}_{NNN}>>` the member text uses.
- **Customer-asset indicator present in multiple member events.** Reconcile to the **lowest event index `i`** in `input_event_refs` (deterministic, and `input_event_refs` is part of `input_hash`, so stable and reproducible). The LLM sees one stable identifier per indicator.
- **Customer-asset indicator with no matching member in this story** (reflection leak — a registered owned-domain none of *this* story's members mentioned). The per-story match finds nothing, so its value is renamed into a **fact scope** `<<REDACTED_{TYPE}_F{k}_{NNN}>>`, where `F` marks the fact namespace and `k` indexes it, with the value carried from the fact's self-scoped map into this story's demap state. No event map is mutated (per-event maps stay immutable, per RFC 0002). Because the match is per-story, the *same* cached fact may reconcile to an `E{i}` in one story and an `F{k}` in another — which is correct, and is exactly why the link is not baked into the cache. This case exists **only** to carry reflected customer assets through to demap, so it is rare.

Because external indicators pass through raw, the `F{k}` fact scope only ever holds **customer-asset indicators that appear solely in a fact** — i.e. the reflection-leak case. It is the one genuinely new piece this amendment adds to the token model; everything else reuses RFC 0002's `E{i}` rewrite. Its exact storage shape (a column on the story result row vs. an `input_event_refs` entry with a `kind` discriminator) is an implementation decision for #424's PR.

**Fact scope is resolved at the story boundary; it never enters report scope.** RFC 0002's report-scope rewrite (§"Report-scope rewrite") only knows how to renamespace `E{i}` / event tokens (keyed on `(aice_id, event_key)` in `input_event_refs`) into `R{j}`. `F{k}` tokens have no `(aice_id, event_key)` and would be unresolvable there. Therefore, when a `story_analysis_result.analysis_text` is consumed as input to a periodic report, any `F{k}` token is **demapped (or re-masked to a stable placeholder) before report-scope rewriting runs** — the report builder never sees a live `F{k}`. This keeps RFC 0002's report-scope pass unchanged (it continues to handle only `E{i}`/event → `R{j}`) and prevents unresolved fact tokens from leaking into report inputs. Demap of `E{i}` tokens in story output continues to reuse RFC 0002's chain unchanged.
...
Like #422, the engine is still pre-release and undeployed, so there is no stored data under an old policy to reconcile. Adding the `domain` KIND, extending domain redaction to event payloads, adding the `hmac` / `hmacKeyVersion` / `normalizationVersion` fields to map entries, and adding the fact pipeline all require **no `ENGINE_VERSION` bump, no policy-version migration, and no retroactive re-redact**. The change is made in place. (`hmacKeyVersion` and `normalizationVersion` travel on each map entry so a future HMAC-key rotation or normalization-rule change stays verifiable per entry; they are not folded into `computePolicyVersion`, which tracks the redaction *policy* — ranges and owned-domains — not the reconciliation key.)
...
|---|---|---|---|
| 1 | Schemas + permission seeds | All new tables (`event_redaction_map`, **`enrichment_redaction_map`** (Amendment A — self-scoped, **story-agnostic** encrypted map for stored enrichment facts, parallel to `event_redaction_map`; holds redacted fact text reference, encrypted map `fact_token → {kind,value,hmac,hmacKeyVersion,normalizationVersion}`, and fact `redaction_policy_version`. Carries **no** story/member data — fact↔member reconciliation is computed per story at prompt-build via `hmac` match, not stored here), `event_analysis_result`, `customer_redaction_ranges`, **`customer_owned_domains`** (Amendment A — `customer_id` + normalized owned-domain suffix), `customer_retention_policy`, `redaction_jobs`). Existing-table column changes: Phase 1 `detection_events` per-event restructure; Phase 2 `policy_event.orig_addr`/`resp_addr` `INET → TEXT`; add `redaction_policy_version TEXT NOT NULL` to `detection_events`, `baseline_event`, `story_member`, `policy_event` (and the new `event_analysis_result`). Extend the redaction-map entry shape (both `event_redaction_map` and `enrichment_redaction_map`) with `hmac` / `hmacKeyVersion` / `normalizationVersion` (Amendment A — used for plaintext-free fact↔member reconciliation at prompt-build). Add and verify dedicated permission seeds for `customer-redaction-ranges:read/write`, **`customer-owned-domains:read/write`**, and `customer-retention:read/write` (User/Analyst/Manager/Admin for read, Manager/Admin for write) | — |
| 2 | Redaction engine module | Pure module: regex scanning, IP/CIDR matching, customer range loading, **owned-domain suffix loading + matching (Amendment A `domain` KIND, applied to payloads and facts)**, token assignment, map structure including **per-entry `hmac` computation (keyed HMAC of the normalized value, with `hmacKeyVersion`/`normalizationVersion`) for fact↔member reconciliation**. Unit tests cover policy completely. No DB writes, no HTTP | 1 (schemas referenced by tests) |
