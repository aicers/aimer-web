...
**Storage home.** This requires a concrete place to persist, per stored fact, parallel to how an event stores them: the **redacted fact text**, the **encrypted fact redaction map**, the fact's **`redaction_policy_version`**, and — for each customer-asset token — the **member-resolution link** `(aice_id, event_key, event_token)` or null (used by step 2; see below). Introduce an `enrichment_redaction_map` table (or equivalent columns on the enrichment cache/result row) in the same shape as `event_redaction_map` plus the link, encrypted via the same DEK/KEK envelope, keyed to the enrichment row rather than `(aice_id, event_key)`. Demap at render time uses it exactly as event demap uses the per-event map. Without this home the DB-write-boundary rule cannot be implemented or demapped; it is part of this amendment's scope (see the sub-issue breakdown).

**Step 2 — rename at prompt-build (read only, never re-redact).** RFC 0002 (§"multi-event token namespacing") assembles a story prompt by renaming each event's per-event tokens `<<REDACTED_{TYPE}_{NNN}>>` to **story-scope** `<<REDACTED_{TYPE}_E{i}_{NNN}>>`, where `i` indexes the event in `input_event_refs`. Fact tokens are reconciled in the **same pass**. "Read only, never re-redact" means no *raw value is re-scanned or re-classified* — but reconciling a self-scoped fact token to a member's `E{i}` token does require deciding the two denote the **same indicator**, and a fact token and an event token are different strings.

That equality is established by an **explicit link recorded at fact write time**, not by re-deriving it at build. When the enrichment worker redacts a fact (step 1), it already holds, per customer-asset indicator, the `(aice_id, event_key)` and per-event token that the indicator resolved to in the member it was matched against (the worker obtained the indicator by decrypting that event's map — see RFC 0003 worker step 1). It persists that resolution as a **link** on the fact's `enrichment_redaction_map` entry: `{ fact_token → (aice_id, event_key, event_token) }`, or null when the indicator matched no member event (the reflection-only case). Prompt-build then simply **follows the link**: a linked fact token is renamed to the member's story-scope `<<REDACTED_{TYPE}_E{i}_{NNN}>>` (look up `(aice_id, event_key)` → index `i` in `input_event_refs`); a null-link customer-asset token goes to fact scope `F{k}`. No map decryption, no value comparison, and **no change to the `event_redaction_map` entry shape** — the link lives only on the fact side. (This is the reconciliation mechanism chosen for #424; a content-addressed alternative — storing a `normalizedIndicatorHmac` on both map entries and matching on it — was considered but rejected because it would change the event map shape and `computePolicyVersion` for a fact-only need.)

Because payload and fact share one write-time policy, a customer-asset indicator that appears in a member event is tokenized in that event's map (for **both** IPs and domains) and the worker has the link, so a linked reconciliation always finds the member token. For each indicator carried by a fact:
...
- **Customer-asset indicator present only in the fact** (reflection leak — a registered owned-domain no member event mentioned). It has no `E{i}` anchor, so its self-scoped fact token is renamed into a **fact scope** `<<REDACTED_{TYPE}_F{k}_{NNN}>>`, where `F` marks the fact namespace and `k` indexes it. The raw value lives only in the fact's encrypted map (written in step 1); no event map is mutated (per-event maps stay immutable, per RFC 0002). This case exists **only** to carry reflected customer assets through to demap, so it is rare.
