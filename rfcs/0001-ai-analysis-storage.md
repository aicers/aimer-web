...
**Storage home.** This requires a concrete place to persist three things per stored fact, parallel to how an event stores them: the **redacted fact text**, the **encrypted fact redaction map**, and the fact's **`redaction_policy_version`**. Introduce an `enrichment_redaction_map` table (or equivalent columns on the enrichment cache/result row) in the same shape as `event_redaction_map` — encrypted via the same DEK/KEK envelope, keyed to the enrichment row rather than `(aice_id, event_key)`. Demap at render time uses it exactly as event demap uses the per-event map. Without this home the DB-write-boundary rule cannot be implemented or demapped; it is part of this amendment's scope (see the sub-issue breakdown).

**Step 2 — rename at prompt-build (read only, never re-redact).** RFC 0002 (§"multi-event token namespacing") assembles a story prompt by renaming each event's per-event tokens `<<REDACTED_{TYPE}_{NNN}>>` to **story-scope** `<<REDACTED_{TYPE}_E{i}_{NNN}>>`, where `i` indexes the event in `input_event_refs`. Fact tokens are reconciled in the **same pass**. "Read only, never re-redact" means no *raw value is re-scanned or re-classified* — but reconciling a self-scoped fact token to a member's `E{i}` token does require deciding the two denote the **same indicator**, and a fact token and an event token are different strings. That equality is decided **without decrypting to plaintext**, by comparing the **`normalizedIndicatorHmac`** (the keyed HMAC RFC 0003 already stores per match / per map entry — see RFC 0003 "Audit / evidence model"): the fact map entry and the event map entry carry the same HMAC iff they are the same normalized indicator. When the HMACs match, the fact token is renamed to that event's `E{i}` token. (Equivalently an implementation may store an explicit link at write time; the HMAC comparison is the default since it needs no extra state.) Because payload and fact share one write-time policy, a customer-asset indicator that appears in a member event is already tokenized in that event's map (for **both** IPs and domains), so this reconciliation always finds the member token. For each indicator carried by a fact:
