...
벤더 IOC 저장소(Unit 42)는 여기에 표시되지 **않습니다**. 저장소는 하나의
...
- **허용 목록 파일만.** Unit 42의 경우 디팡된 `.txt` 지표 목록만
    ...
    프로즈에 담긴 네트워크 IOC는 후속 작업으로 미뤄집니다.
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42나 ESET에는 Auth-Key를
    구성하지도 받지도 않습니다 — GitHub 비인증 속도 제한에 의존합니다(1시간
    케이던스 하한에는 충분). 속도 제한을 높이기 위한 운영자 GitHub 토큰은 이번
    릴리스에서 **연결되어 있지 않으며**, 토큰 지원은 후속 작업으로 미뤄집니다.
