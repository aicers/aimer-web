...
<!-- Screenshot placeholder: 위 캡처는 prodaft/malware-ioc, zscaler/threatlabz,
     ...
     GitHub 토큰 항목도 함께 표시해야 합니다). 같은 재캡처에서 #657이 URL 피드의
     `recent.csv` 내보내기에서 변경한 abuse.ch/urlhaus-payloads 행의 새 엔드포인트
     (`files/exports/…/payload.csv.zip`)도 함께 표시해야 합니다. -->

### 소스별 가져오기 구성

각 소스에는 내장된 가져오기 URL과 **하드 케이던스 하한**(가져오기 사이 최소
간격, 어떤 것도 이를 무시할 수 없음)이 있습니다. 이 하한은 각 업스트림
제공자에 대한 과도한 가져오기를 방지합니다. abuse.ch / Spamhaus "최근"
내보내기는 IP 차단을 막기 위한 것(5분)이고, URLhaus 페이로드 전체 덤프는
6시간 하한(델타가 아닌 대용량 전체 아카이브)이며, Botvrij.eu 및 GitHub에
호스팅된 Phishing.Database 목록은 1시간 예의 하한입니다.
...
| `abuse.ch/urlhaus-payloads` | URLhaus 페이로드 CSV 내보내기, 스트리밍(`files/exports/…/payload.csv.zip`) | 필요 | 6시간 |
...
URLhaus **페이로드** 내보내기는 다른 소스와 형식이 다릅니다. 내부 단일 CSV가
수 기가바이트로 압축 해제되는 **ZIP 아카이브**(`payload.csv.zip`)로, 메모리에
담을 수 있는 한도를 훨씬 넘습니다. 따라서 aimer-web은 버퍼링하지 않고 끝까지
**스트리밍**합니다(ZIP 엔트리 인플레이트 → 한 줄씩 파싱 → 스테이징 테이블
적재 → 한 트랜잭션 안에서 스냅샷 교체). Auth-Key는 URL 경로에 그대로
포함됩니다. 대용량 전체 덤프이므로 케이던스 하한은 **6시간**(URL 피드의 5분이
아님)이고, 수백 메가바이트 다운로드를 감당하도록 소스별 가져오기 타임아웃이
길며, 변경되지 않은 덤프는 조건부 `If-None-Match` / `If-Modified-Since`
재검증으로 건너뜁니다(`304`는 스냅샷을 그대로 둡니다).
...
| --- | --- | --- | --- | --- |
| `unit42/threat-intel` | `PaloAltoNetworks/Unit42-Threat-Intelligence-Article-Information` | The Unlicense(퍼블릭 도메인 — 출처 표기 불필요) | 선택(공유 GitHub 토큰) | 1시간 |
