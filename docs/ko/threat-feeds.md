...
**ESET**, **Volexity** 벤더 IOC 저장소를 관리할 수 있습니다. 또한 **MISP
warninglists** 오탐 억제 계층도 관리합니다([부정 소스 (오탐 억제)](#부정-소스-오탐-억제)
참고). 관리자 사이드바에서 **위협 피드**로 이동하여 엽니다.

평면 Tier-1 피드는 게시된 단일 파일이지만, **벤더 IOC 저장소**(예: Unit 42,
ESET, Volexity)는 보고서별 파일들로 구성된 전체 Git 저장소이며 하나의 단위로
가져옵니다. 벤더 저장소는 **self-fetch 전용**입니다 —
...
벤더 IOC 저장소(Unit 42, Volexity)는 여기에 표시되지 **않습니다**. 저장소는 하나의
...
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42, ESET, Volexity에는
...
- **보고서 맥락.** 가져온 각 지표에는 파일별 GitHub blob URL과 저장소가 인코딩한
    ...
    패밀리(예: `gamaredon`, 또는 대소문자가 섞인 `GhostRedirector`)를 지표의
    출처로 표시합니다.
- **출처 표기.** ESET는 저작권 고지 유지를 요구하는 **BSD-2-Clause**로
    배포되므로, 소스 레이블에 **"ESET (BSD-2-Clause)"** 출처 표기가 기본적으로
