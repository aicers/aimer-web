...
**ESET**, **Volexity**, **PRODAFT**, **Zscaler ThreatLabz**, **Huntress** 벤더
IOC 저장소를 관리할 수 있습니다. 또한 **MISP warninglists** 오탐 억제 계층도
...
벤더 IOC 저장소(Unit 42, ESET, Volexity, PRODAFT, Zscaler ThreatLabz, Huntress)는
여기에 표시되지 **않습니다**. 저장소는 하나의
...
| `huntress/threat-intel` | `huntresslabs/threat-intel` | MIT — **Huntress 출처 표기 유지** | 없음(키리스) | 1시간 |
...
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42, ESET, Volexity, PRODAFT,
    Zscaler ThreatLabz, Huntress에는 Auth-Key를 구성하지도 받지도 않습니다 —
    GitHub 비인증 속도 제한에 의존합니다(1시간 케이던스 하한에는 충분). 속도 제한을
    높이기 위한 운영자 GitHub 토큰은 이번 릴리스에서 **연결되어 있지 않으며**, 토큰
    지원은 후속 작업으로 미뤄집니다.
...
- **출처 표기.** ESET는 **BSD-2-Clause**로, PRODAFT와 Zscaler ThreatLabz, Huntress는
    **MIT**로 배포되며 둘 다 저작권 고지 유지를 요구하므로, 소스 레이블에
    **"ESET (BSD-2-Clause)"**, **"PRODAFT (MIT)"**, **"Zscaler ThreatLabz (MIT)"**,
    **"Huntress (MIT)"** 출처 표기가 기본적으로 포함됩니다(매칭된 지표가 소스를
    인용하는 모든 위치에 표시).
