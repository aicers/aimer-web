...
**ESET**, **Volexity**, **PRODAFT** 벤더 IOC 저장소를 관리할 수 있습니다. 또한
...
ESET, Volexity, PRODAFT)는 보고서별 파일들로 구성된 전체 Git 저장소이며 하나의
단위로 가져옵니다. 벤더 저장소는 **self-fetch 전용**입니다 —
...
벤더 IOC 저장소(Unit 42, Volexity, PRODAFT)는 여기에 표시되지 **않습니다**. 저장소는 하나의
...
<!-- Screenshot placeholder: 위 캡처는 prodaft/malware-ioc 행이 추가되기 전
     상태입니다. PRODAFT를 포함하도록 self-fetch 상태 표를 다시 캡처하는 작업은
     #646에서 추적합니다. -->
...
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42, ESET, Volexity, PRODAFT에는
    Auth-Key를 구성하지도 받지도 않습니다 — GitHub 비인증 속도 제한에
    의존합니다(1시간 케이던스 하한에는 충분). 속도 제한을 높이기 위한 운영자
    GitHub 토큰은 이번 릴리스에서 **연결되어 있지 않으며**, 토큰 지원은 후속
    작업으로 미뤄집니다.
- **보고서 맥락.** 가져온 각 지표에는 파일별 GitHub blob URL과 저장소가 인코딩한
    ...
    행위자 이름과 매핑되지 않음) 지표의 출처로 표시합니다.
- **출처 표기.** ESET는 **BSD-2-Clause**로, PRODAFT는 **MIT**로 배포되며 둘 다
    저작권 고지 유지를 요구하므로, 소스 레이블에 **"ESET (BSD-2-Clause)"**와
    **"PRODAFT (MIT)"** 출처 표기가 기본적으로 포함됩니다(매칭된 지표가 소스를
    인용하는 모든 위치에 표시).
