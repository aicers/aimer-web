...
- **허용 목록 파일만.** Unit 42의 경우 디팡된 `.txt` 지표 목록만
    ...
    아티팩트(파일 경로, 레지스트리 키, DLL 이름)이기 때문입니다.
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42에는 Auth-Key를 구성하지도
    받지도 않습니다 — GitHub 비인증 속도 제한에 의존합니다(1시간 케이던스 하한에는
    충분). 속도 제한을 높이기 위한 운영자 GitHub 토큰은 이번 릴리스에서 **연결되어
    있지 않으며**, 토큰 지원은 후속 작업으로 미뤄집니다.
- **보고서 맥락.** 가져온 각 지표에는 파일별 GitHub blob URL이 포함되며,
    파일명이 클러스터 id(예: `CL-STA-0910`)를 인코딩한 경우 캠페인 id도
    포함되어 지표의 출처로 표시됩니다.
