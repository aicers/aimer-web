...
<!-- Screenshot placeholder: 위 캡처는 prodaft/malware-ioc, zscaler/threatlabz,
     meta/threat-research 행이 추가되기 전 상태입니다. PRODAFT를 포함하도록
     self-fetch 상태 표를 다시 캡처하는 작업은 #646에서, Zscaler ThreatLabz 행은
     #647에서, Meta 행은 #649에서 추적합니다(하나의 재캡처로 함께 처리 가능). -->
...
| `meta/threat-research` | `facebook/threat-research` | MIT (Meta Platforms, Inc.) | 없음(키리스) | 1시간 |
...
- **키리스 가져오기(v1).** 키 없이 가져오며 — Unit 42, ESET, Volexity, PRODAFT,
    Zscaler ThreatLabz, Huntress, Meta에는 Auth-Key를 구성하지도 받지도 않습니다 —
