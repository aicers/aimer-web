...
<!-- Screenshot placeholder: 자체 가져오기 모드의 위협 피드 페이지 — 소스별
     Fetch URL, 마지막 가져오기, 상태, 신선도, 지금 가져오기 버튼이 있는 표
     위에 URLhaus Auth-Key 패널
     (docs/assets/admin-ti-feeds-selffetch-table.png). 재현 방법:
       TI_FEED_MODE=self-fetch pnpm capture --grep 'admin-ti-feeds-selffetch'
     (캡처 스펙은 이 샷들을 TI_FEED_MODE=self-fetch로 게이팅함). -->
...
<!-- Screenshot placeholder: Auth-Key 설정 대화상자(마스킹된 비밀번호 입력)
     (docs/assets/admin-ti-feeds-selffetch-authkey-dialog.png). 재현 방법:
       TI_FEED_MODE=self-fetch pnpm capture --grep 'admin-ti-feeds-selffetch'
     (캡처 스펙은 이 샷들을 TI_FEED_MODE=self-fetch로 게이팅함). -->
