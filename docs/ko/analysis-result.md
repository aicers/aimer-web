...
남아 있는 경우, 페이지에는 "원본 이벤트는 보존 정책에 따라 삭제되었으며,
분석 결과는 보존되었습니다."라는 노란색 배너가 표시됩니다. 이 상태에서는
"원본 이벤트 보기" 단계와 "강제 재실행" 버튼이 모두 숨겨집니다 — 링크할 원본
...
또한 고정된 기본값이 아니라 **현재 보고 있는 변형** — 페이지 URL의 언어와
모델 — 을 재생성합니다. 새 결과가 도착하면 최신 세대를 대체하고(이전 결과
행은 `superseded_at` 스탬프와 함께 보존됨), 페이지는 새 세대로
이동합니다.
...
이미 수집된 레다크션 이벤트를 다시 분석합니다. **강제 재실행**은 대신
*새 원본 이벤트*를 소스에서 다시 수집하고 현재 정책으로 다시 레다크션합니다
— 이는 원본 페이로드가 필요한 작업이며, 그 페이로드는 aice-web-next만
보유합니다. 원본 이벤트나 레다크션 정책이 변경된 경우에는 강제 재실행을,
동일한 입력에 대해 모델을 다시 실행하려면 재생성을 사용하세요.
