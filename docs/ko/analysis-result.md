...
점수 항목 위에 페이지는 **위협 인텔리전스 IOC 증거** 섹션을 표시합니다.
이벤트를 Tier-1 로컬 위협 인텔리전스 피드와 대조하여 일치한 지표에 대한
**판정 배너**와 **피드 소스 출처**로 구성됩니다. 판정은 네 가지 상태를
가집니다.

- **알려진 IOC 탐지됨** — 알려진 침해 지표가 이벤트와 일치했으며, 뒷받침한
  피드는 아래 출처에 나열됩니다.
- **알려진 IOC 없음 (완전 검사)** — 지표가 완전히(`complete` 커버리지)
  대조되었고 아무것도 일치하지 않았습니다. 진정한 정상 결과입니다.
- **알려진 IOC 없음 (미확인)** — 피드를 사용할 수 없거나 오래되었거나 일부만
  커버되어, 일치가 없다는 것이 확인된 정상 결과가 아닙니다.
- **IOC 보강 미실행** — 이벤트에 대한 보강 상태 행이 없어 판정을 사용할 수
  없습니다. 자동 베이스라인을 거치지 않고 수동으로만 분석된 이벤트가 여기에
  해당합니다. 이는 **결코** 정상 결과로 표시되지 않습니다 — "미실행"은
  "완전 검사, IOC 없음"과 명백히 구분됩니다.

판정은 이벤트의 보강 상태 행에서 가져오며, 이 행은 일치한 지표가 0개여도
존재합니다 — 따라서 완전히 검사된 정상 이벤트는 미실행이 아니라 "완전 검사"로
표시됩니다.

지표가 일치하면 각 지표는 뒷받침한 피드와 함께 클래스로 구분되어 인용됩니다 —
**기준 충족**(플로어를 올린 결정적·라이선스 확보 매칭, 두드러지게 표시),
**보조(결정적)**, **보조(평판)** — 그리고 **소스** 레이블, 피드 **버전** /
**해시** 스냅샷, **확인** 시각을 함께 표시합니다. 지표는 비식별화에 일관되게
표시됩니다. 외부 지표는 직접 표시되고, 고객 자산 지표는 이 이벤트 범위
안에서만 복원되며, 복원이 불가능하면 잘못된 값을 노출하지 않고 비식별화
토큰으로 저하됩니다.

IOC 증거는 투명성 표시일 뿐이며, 우선순위 등급·점수·근거를 변경하지 않습니다.

<!-- 스크린샷 자리표시자: 위협 인텔리전스 IOC 증거 섹션이 있는 이벤트 분석
     페이지 — 판정 배너와 피드 소스 출처(소스 레이블, 버전/해시, 확인 시각).
     IOC 증거는 aice-web-next가 공급하는 보강에서 비롯되므로, Tier-1 피드가
     로드되고 지표가 일치한 이벤트가 있는 스택의 실데이터 캡처가
     필요합니다(docs/AUTHORING.md 참조). -->

## CVE 참조

MITRE 칩 아래에는 분석이 이벤트와 연관시킨 CVE에 대한 **CVE 칩** 행이
...
- **확인된 불일치(중대하지 않음)** — 등급이 낮고 CVE와 무관한 분석에서는
  행이 단순히 생략됩니다.
- **검증 불가** — 카탈로그를 사용할 수 없거나, 오래되었거나, CVE 검사가
  오류를 일으킨 경우, 대신 **"CVE 보강 불가"** 알림이 표시됩니다. 여기서
  CVE의 부재는 *검증되지 않은* 상태이며, 확정된 "알려진 CVE 없음" 결과가
  아니므로 위의 주의 서술은 의도적으로 표시되지 **않습니다**.

배포 환경에서 CVE 보강 파이프라인이 활성화되지 않은 경우 CVE 표면은 전부
표시되지 않습니다(칩 행, 상태, 주의 상태 모두 없음).

<!-- 스크린샷 자리표시자: CVE 칩 행이 포함된 이벤트 분석 페이지(칩을 펼쳐
...
- **언어(Language)** — `KOREAN` 또는 `ENGLISH`. 화면에 표시된 분석
  ...
  있습니다. 현재 버전에 대한 사용자 언어가 아직 준비되지 않은 경우 — 한 번도
  생성되지 않았거나, 재생성된 영어 정본을 다시 번역하는 동안 잠시 — 영어(그다음
  가용한 임의 언어)로 대체됩니다.
...
CVE 참조, 그리고 열별 출처 정보(제공자, 모델, 모델 스냅샷, 프롬프트 버전,
세대). CVE 행은 상세 페이지와 동일한 열별 게이팅을 따릅니다
([CVE 참조](#cve-참조) 참고): CVE 검사가 실행되지 않은 열에는 CVE 표면이
표시되지 않고, 검증이 저하된 열에는 빈 칸 대신 "CVE 보강 불가" 알림이
표시됩니다. 두 열 모두 CVE 표면이 없으면 CVE 행 전체가 생략됩니다.
