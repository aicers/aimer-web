...
**저장**을 클릭하면 변경 내용이 저장됩니다. 저장한 날짜·시간 형식 옵션은
현재 세션에 즉시 적용되어, 화면에 이미 표시된 타임스탬프가 새 형식으로 다시
렌더링되며 새로고침이 필요하지 않습니다. 유효하지 않은 값(지원하지 않는
언어, 알 수 없는 시간대, 제공 목록에 없는 형식 로캘, 또는 알 수 없는 시간
표기)은 거부되며 저장되지 않습니다.
