...
이번 릴리스는 환경설정을 *저장*만 하며, 아직 타임스탬프 표시 방식을
변경하지 않습니다. 저장된 시간대가 날짜/시간 형식에 연결되는 이후
릴리스 전까지는 표시되는 타임스탬프가 브라우저/기본 시간대를 계속
사용합니다. 이 설정은 고객별로 정의되며 개인 환경설정과 무관한
리포트 버킷 경계를 변경하지 않습니다.
