...
| **보고서** | `/reports` | 최근 및 고위험 정기 보안 보고서. |
| **위협 스토리** | `/threat-stories` | 최근 및 고위험 위협 스토리. |
| **탐지 이벤트** | `/suspicious-events` | 최근 및 고위험 분석 이벤트. |
...
- **위협 스토리**와 **탐지 이벤트**는 `analyses:read`가 필요합니다.
...
의도적으로 표시하지 않습니다. 위협 스토리와 탐지 이벤트 행은 심각도와
가능성 점수를 표시할 수 있습니다.

탐지 이벤트 행은 이벤트의 **시간과 종류**로 제목이 표시됩니다 —
`{이벤트 시간} · {종류}`(예: `6/3, 2:05 PM · HTTP Threat`, 계정 시간대로
표시) — 종류를 알 수 없으면 시간만, 시간을 알 수 없으면 단순한 `이벤트`
레이블로 대체됩니다. 종류는 aice-web-next와 동일한 친숙한 이름을
사용합니다(예: `HttpThreat` → "HTTP Threat", 두 언어 모두에서 영어로 표시).
불투명한 `event_key`는 제목으로 표시되지 않습니다.
...
![여러 고객 범위에서의 고객 통합 개요 페이지: 왼쪽의 사이드바 범위 선택기와 보고서, 위협 스토리, 탐지 이벤트 섹션이 각각 전용 통합 페이지로 이동하는 "전체 보기" 링크와 함께 표시된 모습](../assets/cross-customer-overview.ko.png)

> **스크린샷 재촬영 대기(#435).** 위 한국어 캡처는 분석 화면의 UI 크롬을
> `next-intl`로 지역화한 #435 이전에 촬영되어, 부제·빈 상태 메시지·"전체
> 보기" 링크 등 주변 UI가 아직 영어로 표시됩니다. 데이터를 담은 화면이라
> 실데이터 aice-web-next 스택에서 다시 촬영해야 하며, 매뉴얼 정책
