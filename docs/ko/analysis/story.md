...
![분석된 각 구성원이 우선순위 등급 배지와 심각도·가능성 점수를 표시하는 스토리 페이지의 구성원 의심 이벤트 목록](../../assets/story-member-events.ko.png)
...
스토리 분석을 다른 모델로 생성하여 결과를 비교합니다. 두 컨트롤 모두
**애널리스트 전용**이며, 다른 [애널리스트 전용 항목](#애널리스트-전용-항목)과
동일하게 게이팅됩니다: 모델 드롭다운은 재생성 버튼과 동일한 조건(애널리스트
**이면서** 브리지 세션이 아님)을 따르고, 읽기 전용 **비교 대상** 선택기와 열별
출처 정보는 해당 고객의 모든 애널리스트에게 표시됩니다. 제공되는 모델은 구성된
