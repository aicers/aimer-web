...
![HIGH 등급 스토리의 분석 페이지로, 우선순위 배지, 심각도 및 신뢰도 점수, 근거 칩, MITRE ATT&CK 기법 칩, Regenerate 버튼이 표시됨](../../assets/story-detail-high.ko.png)
...
분석 본문 아래에는 이 스토리로 상관된 **구성원 의심 이벤트**가 스토리의
...
설명하는 것과 동일한 증거를 해석합니다. 이벤트에 해당 변형의 결과가 있는
경우 카드에는 구성원의 우선순위 등급 배지와 심각도·가능성 점수가
표시됩니다. 해당 변형에 결과가 없는 구성원(예: 보존 정책으로 제거됨)은
점수 대신 "기본 변형에서 이벤트 분석을 사용할 수 없음"이라는 짧은 안내와
함께 여전히 id로 연결됩니다.
...
<!-- 스크린샷 자리표시자(#396): 구성원 의심 이벤트 목록이 표시된 스토리
페이지. 실제 스토리 구성원 데이터가 있는 스택에서 캡처할 것. -->

## 인용한 리포트(Cited by)

하나 이상의 주기 리포트가 이 스토리를 인용한 경우, 페이지에는 해당
...
<!-- 스크린샷 자리표시자(#396): "Cited by" 추적이 표시된 스토리 페이지.
실제 리포트 인용 데이터가 있는 스택에서 캡처할 것. -->
...
![재생성 확인 모달로, 새 LLM 호출이 발생하고 최신 세대가 슈퍼시드된다는 경고와 Cancel, Regenerate 버튼이 표시됨](../../assets/story-regenerate-modal.ko.png)
