...
단위입니다. 각 스토리는 백그라운드 워커에 의해 두 언어로 분석됩니다.
네이티브로 생성되는 **영어 정규본(canonical)** 과, 그 정규본을 번역해
파생하는 앱 **사용자 언어** 변형입니다. 페이지는 뷰어의 언어로 가장 최신의
비-슈퍼시디드(latest non-superseded) 결과를 렌더링하며,
[언어 전환기](#표시-언어)로 두 언어를 오갈 수 있습니다.
...
2. 디스패처는 이거(eager) 집합의 각 언어 — 영어 기준선과 앱 사용자 언어 —
   에 대해 실제 `story_analysis_job` 행을 비어 있는 모든 `ready` 또는 `dirty`
   상태 행에 대해 시드한 뒤, `FOR UPDATE SKIP LOCKED`로 `queued` 행을
   선택하며 `(customer_id, story_id)` 단위 어드바이저리 락을 적용합니다. (앱
   언어가 영어이면 집합은 단일 영어 변형으로 축약되고 번역은 일어나지
   않습니다.)
3. **영어 정규본**은 네이티브로 생성됩니다. 워커는 정규(canonical) 스토리
   버전(최신 `received_at`)의 멤버를 읽고, 이벤트 범위 리댁션 토큰을 스토리
   범위 토큰(`<<REDACTED_*_E{i}_*>>`)으로 다시 작성한 다음, mTLS로
   `system:analysis-worker` 신원으로 aimer의 `analyzeStory` 뮤테이션을
   호출합니다.
...
5. **사용자 언어** 변형은 네이티브로 생성되지 않고 항상 영어 정규본에서
   파생됩니다. 워커는 정규본이 존재할 때까지(재시도 예산을 소비하지 않고)
   defer한 뒤, aimer의 `translateAnalysisNarrative` 뮤테이션을 호출해 서술과
   점수 근거 문구를 번역합니다. 숫자 심각도·가능성 점수, 우선순위 등급,
   MITRE 기법 코드, 인용된 구성원 참조는 **정규본에서 그대로 복사**되므로 두
   언어의 숫자는 항상 일치하고 사람이 읽는 텍스트만 달라집니다. 번역된 서술과
   근거 문구는 행을 기록하기 전에 정규본의 토큰을 기준으로 누출 스캔을
   거칩니다.
...
스토리는 서버 기본값이 아니라 페이지 로케일에서 해석된 **뷰어의 언어**로
표시됩니다. 헤더의 **언어 전환기**는 지원 언어(영어 / 한국어)를 나열하며,
하나를 선택하면 URL에 `?lang=en` / `?lang=ko`(공유 가능)가 설정되고 페이지가
해당 변형으로 다시 렌더링됩니다. 링크 어휘는 앱 로케일 형식(`en` / `ko`)으로,
주기 리포트 리더와 동일하므로 두 화면 사이에서 링크가 일관되게 전달됩니다.

요청한 언어가 아직 생성되지 않은 경우 — 예를 들어 스토리가 막 준비된 직후
사용자 언어 번역이 아직 진행 중일 때 — 페이지는 영어 기준선으로 폴백하고
요청한 언어 분석을 아직 사용할 수 없다는 짧은 안내를 표시합니다. 이 폴백은
일시적이며, 번역은 워커 틱 한두 번 안에 도착해 이후에는 요청한 언어가 바로
렌더링됩니다.

언어 간에 다른 것은 **서술과 점수 근거 문구뿐**입니다. 심각도·가능성 점수,
우선순위 등급, MITRE ATT&CK 기법 코드는 두 언어에서 동일하므로(사용자 언어
행이 영어 정규본에서 복사) 언어를 전환해도 숫자, 등급 배지, 우선순위 우선
[위협 스토리](threat-stories.md) 목록에서의 스토리 위치는 바뀌지 않습니다. 이는
주기 리포트의 일관성도 유지합니다. 이 스토리를 인용하는 리포트는 리포트
자신의 언어와 무관하게 동일한 리프 점수를 롤업합니다.

<!-- 스크린샷 자리표시자: 언어 전환기(영어 / 한국어)가 있는 스토리 헤더와,
     사용자 언어 변형이 아직 번역 중일 때 표시되는 폴백 안내. 스토리 분석은
     aice-web-next에서 받은 데이터를 렌더링하므로, docs/AUTHORING.md에 따라
     이중 언어 스토리가 적재된 실제 데이터 스택에서 캡처해야 합니다. -->
...
## 위협 인텔리전스 IOC 증거(Threat-intel IOC evidence)

`known_ioc_hit` 플로어 신호는 스토리의 관측된 지표를 Tier-1 로컬 위협
인텔리전스 피드와 대조하여 산출됩니다(see [우선순위와 점수](#우선순위와-점수)).
페이지는 그 작업을 점수 항목 위에 **위협 인텔리전스 IOC 증거** 섹션으로
표시합니다. 투명성을 위해 항상 표시되는 **판정 배너**와, 일치한 지표에 대한
**피드 소스 출처**가 이어집니다.

### 판정 배너

신호는 평가된 커버리지만큼만 신뢰할 수 있으며 "일치 없음"은 매우 다른 두 가지를
의미할 수 있습니다. 배너는 네 가지 상태로 그 구분을 명확히 합니다.

- **알려진 IOC 탐지됨** — 알려진 침해 지표가 Tier-1 피드와 일치했습니다.
  `known_ioc_hit` 플로어를 올린 매칭이며, 일치한 피드는 아래 출처에 나열됩니다.
- **알려진 IOC 없음 (완전 검사)** — 지표가 피드와 완전히(`complete` 커버리지)
  대조되었고 아무것도 일치하지 않았습니다. 진정한 정상 결과입니다.
- **알려진 IOC 없음 (미확인)** — 피드를 사용할 수 없거나 오래되었거나 일부만
  커버된 상태에서 검사가 수행되어, 일치가 없다는 것이 확인된 정상 결과가
  **아닙니다**. 이 경우 `LOW`/`MEDIUM` 스토리의 IOC 차원을 신중하게 다루십시오.
- **IOC 보강 미실행** — 스토리의 현재 표준 버전에 대해 보강이 실행되지 않았거나
  완료되지 못해 판정을 사용할 수 없습니다. 이는 "완전 검사, IOC 없음"과
  명백히 구분됩니다 — 미실행 상태는 **결코** 정상 결과로 표시되지 않습니다.

판정은 스토리의 **현재 표준 버전**을(페이지의 나머지와 동일한 규칙으로 해석)
반영하며 보강 상태 행에서 가져옵니다. 이 행은 일치한 지표가 0개여도 존재하므로,
완전히 검사된 정상 스토리는 미실행 상태로 떨어지지 않고 "완전 검사"로 표시됩니다.
우선순위 등급, 점수, 근거는 판정에 의해 **변경되지 않습니다** — 플로어는 불리언
신호만 읽으며, 이 섹션은 그 위에 얹힌 투명성 표시일 뿐 등급 산출의 입력이
아닙니다.

### 피드 소스 출처

지표가 일치하면 각 지표는 이를 뒷받침한 피드와 함께 클래스로 구분되어
인용됩니다.

- **기준 충족(Floor-supporting)** — 라이선스가 확보된 피드의 결정적 매칭으로,
  `known_ioc_hit`를 올린 매칭이며 두드러지게 표시됩니다.
- **보조(결정적)** — 플로어를 올리지 않는 소스(예: 비공개 지표)의 결정적
  매칭으로, 맥락으로 표시됩니다.
- **보조(평판)** — 맥락을 위해 표시되는 더 약한 평판 매칭으로, 플로어를
  올리지 않습니다.

각 출처는 **소스**(사람이 읽을 수 있는 피드 레이블, 피드가 이미 폐기되었다면
원래 소스 id), 대조된 피드 **버전** / **해시** 스냅샷, **확인** 시각을
표시합니다. 지표 자체는 비식별화에 일관되게 표시됩니다. 외부 지표(공격자
호스트, 공개 IP)는 직접 표시되고, 고객 자산 지표는 기록된 이벤트 범위 안에서만
원본 값으로 복원됩니다. 복원이 불가능하면 잘못된 값을 노출하지 않고 비식별화
토큰으로 표시가 저하됩니다.

<!-- 스크린샷 자리표시자: 위협 인텔리전스 IOC 증거 섹션이 있는 스토리 헤더 —
     판정 배너와, 소스 레이블·버전/해시·확인 시각을 보여주는 피드 소스 출처.
     IOC 증거는 aice-web-next가 공급하는 보강에서 비롯되므로, Tier-1 피드가
     로드되고 지표가 일치한 스토리가 있는 스택의 실데이터 캡처가
     필요합니다(docs/AUTHORING.md 참조). -->
...
MITRE 칩 아래에는 분석이 스토리와 연관시킨 CVE에 대한 **CVE 칩** 행이
표시됩니다. CVE는 탐지 자체에서 나오지 않습니다 — 탐지는 이상 징후/신규성
기반이며 CVE 식별자를 포함하지 않습니다 — 따라서 각 CVE는 LLM의 가설로
시작하여, 표시되기 전에 권위 있는 카탈로그(NVD, CISA KEV, FIRST EPSS)에
대해 **검증**됩니다. 카탈로그에 존재하지 않는 가설 CVE는 폐기되며(감사
로그에 기록됨), 따라서 화면에 표시되는 모든 칩은 실제로 카탈로그에서
확인된 CVE입니다.

각 칩에는 정규 CVE id(예: `CVE-2024-3400`)가 표시되며, 해당 CVE가 CISA의
알려진 악용 취약점(Known Exploited Vulnerabilities) 목록에 있으면 `KEV`
표식이 함께 표시됩니다. 칩을 펼치면 각 값의 출처가 인라인으로 인용된
보강 페이로드가 나타납니다. 예:

> `CVSS 9.8 [NVD] · KEV [CISA] · EPSS 0.94 / p99 [FIRST]`

여기에 한 줄 요약과, CVE를 확인한 출처를 명시하는 **검증 출처** 푸터가
더해집니다. `KEV` 표식이 이미 전달하지 않은 실제 악용(in-the-wild) 신호는
`In the wild [CISA]`로 표시됩니다. 인라인 `[NVD]` / `[CISA]` / `[FIRST]`
인용과 푸터는 모든 값의 출처이므로(상위 출처 표기 요건도 이로써
충족됩니다), 칩은 보이는 검증 출처 없이 펼쳐지지 않습니다.

### CVE가 적용되지 않을 때

어떤 CVE와도 일치하지 않는 위협은 정상적이고 예상된 결과입니다 — 플랫폼은
CVE 일치를 절대 강제하지 않습니다. 빈 상태의 표시 방식은 CVE 검사가 실제로
완료되었는지에 따라 달라집니다.

- **확인된 불일치(중대한 위협)** — 카탈로그를 사용할 수 있었고 분석이
  중대한 경우(`HIGH`/`CRITICAL` 등급)인데도 검증된 CVE가 없으면, 페이지에
  **"알려진 CVE 연관 없음"** 주의 메시지가 표시됩니다. 이 부재 자체가
  의미를 가지며, 공개된 CVE가 없는 신규·변종·제로데이 위협일 수 있습니다.
  안전하다고 단정하지 말고 더욱 주의하십시오.
- **확인된 불일치(중대하지 않음)** — 등급이 낮고 CVE와 무관한 분석에서는
  행이 단순히 생략됩니다.
- **검증 불가** — 카탈로그를 사용할 수 없거나 오래된 경우, 대신 **"CVE
  보강 불가"** 알림이 표시됩니다. 여기서 CVE의 부재는 *검증되지 않은*
  상태이며, 확정된 "알려진 CVE 없음" 결과가 아니므로 위의 주의 서술은
  의도적으로 표시되지 **않습니다**.

배포 환경에서 CVE 보강 파이프라인이 활성화되지 않은 경우 CVE 표면은 전부
표시되지 않습니다(칩 행, 상태, 주의 상태 모두 없음).

<!-- 스크린샷 자리표시자: CVE 칩 행이 포함된 스토리 헤더(칩을 펼쳐
     CVSS / KEV / EPSS 페이로드 + 출처 인용을 표시). CVE 파이프라인이
     활성화되고 실제 CVE 카탈로그가 로드된 스택에서의 실데이터 캡처가
     필요합니다(docs/AUTHORING.md 참조). -->
...
- **언어(Language)** — 현재 표시 중인 분석의 언어(`ENGLISH` 또는 `KOREAN`).
  모든 뷰어에게 표시되며, 헤더의 [언어 전환기](#표시-언어)로 변경할 수
  있습니다.
...
[분석 결과 페이지](../analysis-result.md)로 연결되는 카드이며, 스토리의 표시
언어를 따라가되(해당 이벤트 변형이 존재할 때) 없으면 영어로 폴백하므로 이벤트
페이지가 카드가 설명하는 것과 동일한 증거를 해석합니다. 카드 제목은 이벤트의 시간과 종류 —
...
- 강제 재실행은 이중 언어 쌍 전체를 대상으로 합니다. 영어 정규본 **과**
  사용자 언어 변형이 동일한 새 `generation`으로 올라가고, `status`는
  `queued`, `attempts`는 `0`으로 초기화되며 다음 워커 틱부터 작업이
  시작됩니다. 정규본은 네이티브로 재생성되고 사용자 언어 행은 그로부터 번역을
  다시 파생하므로, 슈퍼시드된 영어 세대에 고정된 채 남는 번역 행은 없습니다.
  요청은 즉시 반환됩니다(번역 완료를 기다리며 블로킹하지 않습니다).
