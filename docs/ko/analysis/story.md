...
- **신뢰도 점수(Likelihood score)** — `0.000`–`1.000` 범위, 소수점 세 자리.
  ...
  멤버가 다섯 개 이상이면 매트릭스 룩업 직전에 신뢰도를 `≥ 0.7`로 상승). 이
  덕분에 보정 데이터와 플로어 정책을 변경할 때 과거 데이터를 재기록할
  필요가 없습니다.
...
본문에는 스토리 범위 토큰(`<<REDACTED_*_E{i}_*>>`)이 그대로 보존된
LLM 분석 텍스트가 표시됩니다. 토큰 네임스페이싱은 LLM이 멤버 이벤트를 가로질러
엔티티를 잘못 병합하는 것을 방지합니다. 분석가 UI는 토큰을 평문으로
복원하지 않고 그대로 노출하므로, 매핑되지 않은 잔여 토큰(환각의 신호)을
즉시 발견할 수 있습니다. 환각으로 판단된 디코딩은 기록 시점에 차단되며 이
화면에 도달하지 않습니다.
