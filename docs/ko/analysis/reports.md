...
> 캡처(aicers/gauntlet#149, #365)입니다. #450 이전의 캡처는 아직 옛
> 헤더(별도의 집계 점수 행)를 보여 주었으므로, 오래되었거나 조작된
> 캡처가 실 캡처를 대신해서는 안 된다는 매뉴얼 정책에 따라 실데이터
> 스택에서 새로 촬영(#429와 공유하는 제약)할 수 있을 때까지
> 플레이스홀더로 교체했습니다.
