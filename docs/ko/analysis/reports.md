...
[탐지 이벤트](suspicious-events.md), 그리고 해당 구간의 탐지 이벤트
동향을 함께 엮습니다. 스토리
분석(하나의 스토리에 대한 한 번의 LLM 호출)과 달리, 리포트는 여러 개별
분석을 하나의 서술로 집계하며 LLM에게 점수를 요청하지 **않습니다**. 리포트의
우선순위는 포함된 스토리·이벤트 분석과 탐지 이벤트 동향으로부터 Clumit Insight가
직접 도출합니다.
...
![정기 리포트 상세 페이지로, 우선순위 등급 배지와 그 출처 안내, 집계 심각도 및 가능성 점수, MITRE ATT&CK 기법 칩, 그리고 요약·스토리 하이라이트·주요 이벤트·탐지 이벤트 동향·기간 전망 섹션이 표시됨](../../assets/report-detail.ko.png)
...
제외)에는 **달력 버튼**이 있으며, 이 버튼은 그 자리에서 앵커형 **달력
팝오버**(아래)를 열고, 해당 주기의 가장 최근 리포트가 속한 뷰포트(일간은 월,
주간/월간은 연도)를 중심으로 표시합니다. 이 버튼은 독립 달력 페이지로 가는
링크이기도 하므로 JavaScript 없이도 동작합니다.

## 리포트 달력

색인의 최근 목록은 의도적으로 짧습니다. **달력**은 더 오래된 리포트로 가는
선택적 경로이며, 서브젝트의 기본 진입 지점은 항상 가장 최근 리포트입니다.
달력은 두 가지 방법으로 도달할 수 있고, 둘 다 **동일한 그리드**(동일한 셀
상태, 스타일, 범례)를 렌더링합니다.

- **인컨텍스트 팝오버**(일상 경로) — 리포트 색인(Live 제외 주기 섹션마다)과
  리포트 상세 툴바의 **달력 버튼**이 그 자리에서 해당 주기의 그리드를 보여주는
  앵커형 팝오버를 엽니다. 팝오버는 기본적으로 접혀 있으며 **한** 주기의
  그리드만 필요할 때 렌더링합니다. 네 주기를 인라인으로 펼치지 않습니다.
  뷰포트 이전/다음(한 달 또는 한 해)은 **전체 페이지 이동 없이** 팝오버 안에서
  가져오며, 리포트가 있는 버킷을 선택하면 그 상세 페이지가 열립니다.
- **독립 페이지**(주소 지정 경로) — `…/[period]/calendar`의 동일한 달력으로,
  공유/딥링크/비-JS 폴백 대상으로 유지됩니다. 달력 버튼의 링크가 이 페이지를
  가리키므로 팝오버가 동작하지 못할 때(JavaScript 비활성, 또는 버튼을 새 탭에서
  열 때)에도 직접 이동이 가능합니다.
...
- **보고서 있음** — 강조 표시되고 클릭 가능합니다. 버킷이 "보고서 있음"이
  ...
  해석되어야 하며, **여기에 더해** 추적 상태가 `ready` 또는 `dirty`여야
  합니다 — 완료된(또는 갱신 중인) 리포트를 나타내는 두 상태입니다. 아직
  생성 중인(`pending`, 오래된 결과가 리포트 저장소에 떠 있더라도) 버킷,
  `ready`/`dirty` 상태이지만 볼 수 있는 결과가 없는 버킷, 또는 추적 상태가
  없거나 아카이브되었는데도 리포트 저장소에 결과만 남아 있는 버킷은
  "보고서 있음"이 **아닙니다**. 이전/다음 이동과 동일한 규칙이라, 달력에서는
  준비된 것처럼 보이는데 열어 보면 렌더링되지 않는 버킷으로 이동하는 일이
  없습니다.
...
- **오래된 쪽** — 더 오래된 리포트가 있지만 보관 경계를 지나 만료된
  경우, 이전 컨트롤은 링크 대신 비활성화된 **보관된 이전 보고서가
  없습니다** 상태를 표시합니다. 가장 첫 리포트(더 오래된 것이 아예 없거나
  보관이 무제한인 경우)에서는 이전 컨트롤이 아예 표시되지 않습니다 —
  경계 메시지는 보관이 실제로 더 갈 곳이 없는 원인일 때만 나타납니다.
- **최신 쪽** — 가장 최근 리포트에서는 다음 컨트롤이 아예 표시되지
  않습니다.

날짜로 바로 이동하려는 경우를 위해 컨트롤 옆에 **달력 버튼**이 있습니다. 이
버튼은 위에서 설명한 동일한 달력 팝오버를 열며, 지금 보고 있는 리포트가 속한
뷰포트(월 또는 연도)를 중심으로 표시합니다. 주기 탭, 주기 내 이전/다음, 그리고
이 달력 버튼은 리포트 본문 위의 단일 **날짜 툴바**로 통합됩니다 — 이 달력
버튼은 리포트 색인이 섹션마다 임베드하는 것과 동일한 컨트롤이므로 두 화면이
일관됩니다. **실시간(Live)**은 단일 롤링 버킷이므로 이전/다음과 달력 버튼이
없으며, 거기서는 툴바에 탭만 표시됩니다.
...
3. 입력 빌더는 **상위 스토리**(스토리 상태가 `ready`이고 슈퍼시드되지 않은
   ...
   이벤트**(디듀프된 탐지 이벤트 시각이 버킷 안에 있으며, 선택된 스토리가 이미
   ...
   인용되며(나머지는 조용히 누락되지 않고 분석된 윈도 집합에 남아 구간의
   [long-tail 서술](#long-tail-서술)을 통해 드러남), 상한에 못 미치면
   ...
   **대체 모델** 리포트는 엄격하게 동작하여 해당 모델의 리프만 선택합니다.
   언어는 항상 엄격하게 일치시킵니다. 또한 구간의 **탐지 이벤트 집계**(중복
   제거된 이벤트 수와 카테고리 분포)와 [long-tail 서술](#long-tail-서술)을
   구동하는 **분석 이벤트 집계**를 계산합니다. 스토리에 포함되지 않은 분석된
   윈도 집합 전체 — 인용된 이벤트와 그 외 윈도 내에서 분석된 모든 이벤트 — 에
   대해 빌더는 분석 총수, 그중 인용된 수, 기법 리더보드와 등급 분포, *비인용*
   나머지에 대한 기법 롤업, 그리고 최대 10개의 기법 군집 예시 표현을 집계합니다.
   **스토리에 포함되지 않은 분석 이벤트가 전혀 없는**(분석된 윈도 집합이 비어 있는)
   윈도만 이 부분을 완전히 생략하므로 해당 리포트는 이전과 동일하게 표시됩니다.
   분석된 이벤트가 있으나 모두 인용된 윈도는 이 부분을 여전히 전송하며 — 분석
   총수가 인용 수와 같아지고 *비인용* 롤업이 비며 예시가 없을 뿐 — long-tail
   서술만 없을 뿐 그 외에는 변동이 없습니다.
...
- **우선순위 등급** — `CRITICAL`, `HIGH`, `MEDIUM`, `LOW` 중 하나로,
  ...
  구간의 탐지 이벤트 동향이 매핑되는 등급 중 **최댓값**입니다. (집계 점수가
...
구간의 **탐지 이벤트 동향**은 윈도의 활동이 직전 비교 구간에서 벗어날 때
리포트의 우선순위를 높일 수 있습니다. 동향 자체는 리포트의 **탐지 이벤트
...
집계 심각도·가능성 점수는 리포트 자체 모델과 일치하는 리프**(및 탐지 이벤트
...
기법 칩 옆에 리포트의 `aggregate_cve_refs`(포함된 각 개별 분석의 CVE 참조의
중복 제거·정렬된 합집합)가 표시됩니다. 각 칩은 표준 CVE 식별자(예:
`CVE-2024-12345`)를 보여줍니다. 기법 컬럼과 똑같이, 이 합집합은 작성 시점에
개별 스토리·이벤트 분석으로부터 결정론적으로 계산되므로 LLM이 컬럼에서 CVE를
추가하거나 제거할 수 없습니다. 기간 단위 표면은 리프 분석이 이미 확정한 내용을
롤업할 뿐입니다. 인용된 CVE가 하나도 없는 기간에는 칩이 표시되지 않습니다.
CVE별 상세 정보(CVSS, KEV, EPSS, 검증 출처)는 이 롤업된 기간 뷰가 아니라 개별
스토리·이벤트 분석 페이지에 표시됩니다.

![정기 리포트 상세 페이지의 집계 CVE 칩 행 플레이스홀더로, 실데이터 재촬영 대기 중 — 이 행은 리프 분석에 CVE 참조가 있을 때만 렌더링되며, 이는 CVE 소스 카탈로그(#601)와 엔리치먼트 게이트에 의존하므로 픽스처 파이프라인으로는 재현할 수 없음](../../assets/report-aggregate-cve.ko.png)
...
- **탐지 이벤트 동향(Detection trends)** — 구간의 탐지 이벤트 카운트와
  순위, 그리고 상위 기법·센서 대비 관찰되는 변화에 대한 짧은 사실 서술. 윈도가
  개별 인용된 이벤트 외에 추가로 분석된 이벤트를 포함한 경우, 이 섹션은
  **long-tail 서술**(아래 참고)도 함께 엮습니다.
...
탐지 이벤트 동향은 여러 항목의 목록이며 페이지는 이를 하나의 블록으로 합쳐
...
**주요 이벤트** 섹션은 개별 인용된 소수의 이벤트만 나열합니다. 그러나 바쁜
윈도는 인용하는 것보다 훨씬 많은 탐지 이벤트를 분석할 수 있습니다 — 모든 `LOW`
이벤트, 그리고 인용 상한을 넘긴 `CRITICAL`·`HIGH`·`MEDIUM` 이벤트가 그렇습니다.
이 신호를 버리는 대신, 리포트는 스토리에 포함되지 않은 분석된 윈도 집합 전체를
**long-tail 서술**로 접어 넣어 **탐지 이벤트 동향** 섹션이 산문으로 엮습니다.
지어낸 세부가 아니라 빌더가 계산한 집계에 근거합니다:

- **분석 수 대 인용 수** — 예를 들어 "이번 윈도에서 분석된 240건 중 위에 10건을
  상세히 다루며, 나머지 230건은…" 처럼 서술할 수 있습니다.
- 전체 분석 집합에 대한 **기법 리더보드와 등급 분포**, 그리고 *비인용* 나머지에
  대한 **기법 롤업** — long-tail이 무엇으로 이루어졌는지 특징지을 수 있습니다.
- **최대 10개의 기법 군집 예시** — 우세한 MITRE 기법마다 하나씩, 각자 도달한
  최고 등급, 대표하는 비인용 이벤트 수, 대표 근거 표현 하나를 담습니다. 개별
  분석을 인용하지 않고도 산문을 구체적 예시로 뒷받침하며, 출처(Sources) 카드로는
  렌더링되지 *않습니다*. (기법이 10개를 넘으면 등급·빈도 상위 10개만 유지하고
  잘림은 로그로 남깁니다.)

이 수치들은 모두 aimer-web이 이미 생성한 분석에서 나오고, 인용된 리프와 동일하게
리댁션 토큰으로 재작성되며, 권한 있는 열람자에게는 같은 경로로 평문으로
복원됩니다. 인용 집합을 넘어 분석된 이벤트가 없는 윈도는 long-tail 서술을 갖지
않으며, 리포트는 이 섹션이 존재하기 이전과 똑같이 읽힙니다. 영문 리포트가 예시
집합과 토큰 번호 매김을 소유하며, 비영문 변형은 영문 집계를 그대로 재사용하므로
(long-tail 수치와 예시 표현이 영문 리포트와 일치) 인용 서술만 해당 언어로
유지됩니다.

> _스크린샷 예정: long-tail 서술은 aice-web-next에서 파생된 데이터를
> 렌더링하므로, 대표 캡처는 지어낸 데이터가 아니라 실제 데이터 스택에서
> 가져옵니다._
...
하나**의 분석 리프로 연결되는 인라인 인용을 가질 수 있습니다. 링크는 위협
스토리의 경우 작은 **↗ Story {story_id}** 칩으로, 탐지 이벤트의 경우
**↗ {이벤트 시간} · {종류}** 칩(예: `↗ 6/3, 2:05 PM · HTTP Threat`)으로
단위 텍스트 뒤에 표시됩니다. 이는 [출처](#출처) 카드 및 탐지 이벤트 목록과
동일한 이벤트 레이블입니다. 이벤트 시간을 알 수 없으면 단순한 **↗ 이벤트**
레이블로 대체되며, 불투명한 `event_key`는 표시되지 않습니다. 이로써 신뢰
사슬이 리포트 수준 출처 패널("어떤 분석이 이 리포트에 기여했는가")에서 문장
수준("어떤 분석이 *이* 주장을 뒷받침하는가")으로 깊어져, 한 문장을 검증할
때 인용된 모든 출처를 훑어볼 필요가 없습니다.
...
- 인용은 세 개의 리프 유래 섹션에만 적용됩니다. **기간 전망**(전망성)과
  **탐지 이벤트 동향**(드릴다운의 의도된 종착점)은 리프 유래가 아니므로 문장
  인용을 갖지 않습니다.
...
리포트가 생성에 사용한 위협 스토리·탐지 이벤트 분석을 나열하는 **출처**
...
![인용된 위협 스토리·탐지 이벤트 카드 위에 "N stories · M events" 출처 줄이 있는 리포트 상세 출처 패널](../../assets/report-sources-panel.ko.png)
...
- **탐지 이벤트 카드**는 이벤트의 시간과 종류 — `{이벤트 시간} ·
  {종류}`(예: `6/3, 2:05 PM · HTTP Threat`)로 제목이 표시되며, 이는 탐지
  이벤트 목록과 동일한 레이블입니다. 출처를 위해 원본 `aice_id`는 제목
  아래의 메타 줄에 표시됩니다. 카드는 우선순위 등급과 심각도·가능성 점수를
  보여 줍니다. 이벤트 시간을 알 수 없으면 단순한 `이벤트` 레이블로
  대체되며, 불투명한 `event_key`는 제목으로 표시되지 않습니다.

스토리에는 사람이 읽을 수 있는 제목이 없으므로 카드는 ID로 표시되며 기법
칩이 사람이 읽을 수 있는 설명을 겸합니다 — 위협 스토리 목록과 동일한
방식입니다.
...
**탐지 이벤트 동향에는 출처 패널이 없습니다.** 이는 드릴다운의 의도된
...
사용자의 시간대에 맞춰 브라우저의 로캘 관례에 따라 표시됩니다
...
등급, MITRE ATT&CK 태그, 언어, 집계 점수, 탐지 이벤트 동향 힌트, 생성된 본문
...
![재생성 확인 모달로, 구간의 스토리·이벤트·탐지 이벤트 동향 전반에 새 LLM 호출이 발생하고 최신 세대가 슈퍼시드된다는 경고와 취소, 재생성 버튼이 표시됨](../../assets/report-regenerate-modal.ko.png)

확인 모달은 구간의 분석된 스토리·이벤트·탐지 이벤트 동향 전반에 새 LLM
...
리포트 섹션(요약, 스토리 하이라이트, 주목할 이벤트, 탐지 이벤트 동향, 기간
...
모델 열에는 나타나지 않음). 종합 섹션(요약, 탐지 이벤트 동향, 기간 전망)은
정상적으로 비교됩니다.
