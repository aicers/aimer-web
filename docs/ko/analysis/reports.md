...
- **보고서 있음** — 강조 표시되고 클릭 가능합니다. 버킷이 "보고서 있음"이
  ...
  해석되어야 하며, **여기에 더해** 아카이브되지 않은 추적 상태가 함께
  있어야 합니다 — 상세 페이지가 렌더링 전에 요구하는 것과 정확히 같은
  조합입니다. 추적만 된(`pending`, 아직 결과 없음) 버킷, `ready`/`dirty`
  상태이지만 볼 수 있는 결과가 없는 버킷, 또는 추적 상태가 없거나
  아카이브되었는데도 리포트 저장소에 결과만 남아 있는 버킷은 "보고서 있음"이
  **아닙니다**. 이전/다음 이동과 동일한 규칙이라, 달력에서는 준비된 것처럼
  보이는데 열어 보면 "생성 중"이나 찾을 수 없음 페이지가 나오는 일이
  없습니다.
