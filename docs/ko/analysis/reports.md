...
![정기 리포트 상세 페이지로, 우선순위 등급 배지와 그 출처 안내, MITRE ATT&CK 기법 칩, 그리고 요약·스토리 하이라이트·주요 이벤트·의심 이벤트 동향·기간 전망 섹션이 표시됨](../../assets/report-detail.ko.png)

> **주간·월간 스크린샷 재촬영 대기(#430, #450).** 위의 일간 캡처는
> 픽스처 기반이라 최신 상태입니다. 우선순위 등급과 그 출처 안내를
> 앞세우며 원시 집계 점수를 더 이상 표시하지 않습니다. 아래의 **주간·월간**
> 변형은 이 픽스처 파이프라인으로 재현할 수 없는 gauntlet 실 파이프라인
> 캡처(aicers/gauntlet#149, #365)라, 아직 옛 헤더(별도의 집계 점수 행)와
> 이 섹션의 옛 제목 "Baseline observations"를 표시합니다. 실데이터
> 스택에서 다시 촬영할 예정이며(#429와 공유하는 제약), 매뉴얼 정책에 따라
> 조작하거나 손으로 편집하지 않고 실 캡처를 이 안내와 함께 유지합니다.
>
> **주간·월간 스크린샷 재촬영 대기(#435).** 위의 일간 한국어 캡처는
> 재촬영되어 UI 크롬이 한국어로 지역화되어 있습니다. 반면 아래의 주간·월간
> 한국어 캡처는 분석 화면의 UI 크롬을 `next-intl`로 지역화한 #435 이전에
> 촬영되어, 제목·부제·섹션 머리글·상태 배지·주기 탭·필터 등 주변 UI가
> 아직 영어로 표시됩니다. 모두 데이터를 담은 화면이라 실데이터
> aice-web-next 스택에서 다시 촬영해야 하며(#430·#429와 공유하는 제약),
> 매뉴얼 정책(docs/AUTHORING.md)에 따라 조작하거나 손으로 편집하지 않고
> 별도 문서 패스에서 재촬영합니다. 영어 캡처는 영어 크롬이 그대로이므로
> 영향을 받지 않습니다.
...
![주간 탭이 활성화된 주간 정기 리포트 상세 페이지 — CRITICAL 한 주 리뷰로, 우선순위 등급 배지와 그 출처 안내, MITRE ATT&CK 기법 칩, 그리고 요약·스토리 하이라이트·주요 이벤트·의심 이벤트 동향(아직 재촬영하지 않은 이 스크린샷에서는 옛 제목인 "Baseline observations"로 표시됨)·기간 전망 섹션이 표시됨. 요약은 매일을 나열하지 않고 한 주의 활동을 하나의 서술로 추상화함](../../assets/report-detail-weekly.ko.png)

![월간 탭이 활성화된 월간 정기 리포트 상세 페이지 — CRITICAL 한 달 리뷰로, 동일한 헤더(우선순위 등급과 그 출처 안내, 기법 칩)와 다섯 개의 서술 섹션이 표시되며, 요약과 의심 이벤트 동향(아직 재촬영하지 않은 이 스크린샷에서는 옛 제목인 "Baseline observations"로 표시됨)이 그 달의 추세를 구간 내에서 서술함](../../assets/report-detail-monthly.ko.png)
...
Clumit Insight는 리포트 정렬을 위해 집계 심각도·신뢰도 값을 내부적으로
계속 도출하지만(`score_kind: "aggregate"`) 이 페이지에는 **표시하지
않습니다**. 점수 결합 방식은 비공개이며, 원시 세 자리 숫자는 "어떻게
산출됐는가?"라는 의문을 부르는 동시에 집계가 지니지 않은 정밀도를
암시하기 때문입니다. 따라서 숫자가 아니라 등급이 리포트의 대표 심각도
신호입니다.

구간의 **의심 이벤트 동향**은 윈도의 활동이 직전 비교 구간에서 벗어날 때
리포트의 우선순위를 높일 수 있습니다. 동향 자체는 리포트의 **의심 이벤트
동향** 섹션에 서술되며, 그 결과인 우선순위 등급만 헤더에 표시됩니다.
...
경우 배지는 우선순위 등급만 담아 이 페이지로 연결되며, 따라갈
때 여는 탭이 선택한 고객과 무관하게 올바른 리포트를 열고, 카드가 표시된 것과
동일한 변형(시간대·언어·제공자·모델)을 엽니다. 배지 자체(섹션 콘텐츠 없이
우선순위 등급만 표시)는 aice-web-next가 렌더링합니다. 해당
스크린샷은 aice-web-next 매뉴얼을 참고하세요.
