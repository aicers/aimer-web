...
스토리의 경우 작은 **↗ Story {story_id}** 칩으로, 의심 이벤트의 경우
**↗ {이벤트 시간} · {종류}** 칩(예: `↗ 6/3, 2:05 PM · HTTP Threat`)으로
단위 텍스트 뒤에 표시됩니다. 이는 [출처](#출처) 카드 및 의심 이벤트 목록과
...
- **의심 이벤트 카드**는 이벤트의 시간과 종류 — `{이벤트 시간} ·
  {종류}`(예: `6/3, 2:05 PM · HTTP Threat`)로 제목이 표시되며, 이는 의심
