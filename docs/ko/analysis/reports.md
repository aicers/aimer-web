...
있으며, 달력은 보관된 모든 버킷에 도달합니다. 이 링크는 현재 월/연도가
아니라 해당 주기의 가장 최근 리포트가 속한 뷰포트(일간은 월, 주간/월간은
연도)로 달력을 엽니다.
...
있습니다. 이 링크는 현재 월/연도가 아니라 지금 보고 있는 리포트가 속한
뷰포트(월 또는 연도)로 달력을 엽니다. **실시간(Live)**은 단일 롤링
버킷이므로 이전/다음이 없으며 거기서는 컨트롤이 표시되지 않습니다.
