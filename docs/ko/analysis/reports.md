...
점수는 포함된 리프와 베이스라인 드리프트로부터 Clumit Insight가 직접 계산합니다.
...
`{period}`는 **대문자**입니다 — `LIVE`, `DAILY`, `WEEKLY`, `MONTHLY`. 경로에
...
이 색인이 생기기 전에는 빈 경로가 `404`를 반환했습니다 — 리포트는 특정
`{period}/{bucketDate}` URL로 직접 이동해야만 열 수 있었습니다. 색인은
사용 가능한 버킷을 주기(Live, Daily, Weekly, Monthly)별로 묶어, 각 주기의
...
**Being generated**(생성 중)로 표시되어 나타납니다. 최신 기본 변형 결과가
있는 버킷은 우선순위 등급과 함께 **Ready**(준비됨)로, 새 소스 데이터로
갱신을 기다리는 버킷은 **Updating**(갱신 중)으로 표시됩니다.
...
구간을 각자의 주기로 보는 리포트로 연결됩니다 — 일간에서 **Weekly**로
전환하면 보고 있던 날이 속한 주가 열리고, **Monthly**는 그 날의 달을 엽니다.
**Live** 탭은 항상 롤링 스냅샷을 엽니다. 추적 중이지만 아직 생성되지 않은
...
베이스라인 드리프트 신호는 구간의 이벤트 카테고리 분포를 같은 길이의 이전
구간(LIVE는 직전 24시간, DAILY는 직전 달력 하루, WEEKLY는 직전 7일,
MONTHLY는 직전 달력 한 달)과 비교합니다.
...
리프에서 파생된 섹션 — 요약·스토리 하이라이트·주요 이벤트 — 아래에는
리포트가 생성에 사용한 위협 스토리·의심 이벤트 리프를 나열하는 **출처**
패널이 표시됩니다. 이는 해당 세대가 기록한 입력 목록이므로, 패널은
**리포트 수준의 인용 출처**입니다. 즉 리포트가 어떤 리프를 근거로 했는지를
알려줄 뿐, 어떤 문장이 어떤 리프를 인용하는지는 나타내지 않습니다. 패널은
문장 단위·주장 단위의 출처를 의미하지 않습니다.

<!-- Screenshot placeholder (#395): 인용된 스토리·이벤트 카드와
     "N stories · M events" 출처 줄을 보여 주는 리포트 상세 출처 패널.
     실제 세대가 있는 스택이 준비되면 캡처. -->

패널 헤더에는 **N stories · M events** 형태의 출처 개수가 표시됩니다.
각 인용 리프는 카드로 렌더링됩니다.

- **위협 스토리 카드**는 `Story {story_id}`로 표시되며, 인용된 리프의
...
원본 언어 리프로 연결되어 고정된 증거가 계속 존재합니다.

인용된 리프의 고정 버전을 더 이상 사용할 수 없는 경우 — 더 새로운 세대로
슈퍼시드되었거나 보존 정책으로 제거된 경우 — 카드는 저장된 ID와 세대만
남기고 **"Evidence version no longer available"** 안내로 축소되며, 카드가
연결하는 리프 상세 페이지도 최신 버전으로 조용히 대체하지 않고 동일한
안내를 표시합니다. 카드의 링크는 여전히 고정된 세대를 담고 있어, 기반
증거 버전이 사라지더라도 출처 추적 정보는 보존됩니다.

**베이스라인 관찰에는 출처 패널이 없습니다.** 이는 드릴다운의 의도된
종착점으로, 해당 섹션은 서술형 베이스라인 관찰만 보고하며 개별 리프로
역추적되지 않습니다.

### 역방향 추적(Cited by)

출처 패널은 링크의 정방향 절반입니다. 역방향 절반은 리프 상세
페이지에 있습니다. 이 리포트가 인용한 [스토리](story.md#인용한-리포트cited-by)나
[이벤트](../analysis-result.md#인용한-리포트cited-by)에는 이 리포트로 다시
올라가는 **Cited by** 추적이 표시됩니다. 이 역방향 링크는 **이 리포트의
세대로 고정**되므로, 리포트 상세 페이지는 `generation` 쿼리 파라미터를
받아들여 최신 버전이 아니라 **정확히 그 세대**를 엽니다. 고정된 리포트
세대가 더 이상 존재하지 않는 경우 — 슈퍼시드되었거나 제거됨 — 페이지는
최신 버전으로 조용히 대체하지 않고 **"This report version is no longer
available"** 안내를 표시하며, 이는 리프 페이지의 고정 증거 동작과
동일합니다.
...
엽니다. 페이지에서와 마찬가지로 `?lang=`은 리포트 언어 열거값이 아니라
**앱 로케일 코드**(`en` / `ko`)입니다. 뷰어 인식 호출자는 해석된 로케일 코드를
전달하며, `?lang=`이 없으면 요약은 (자체적으로 기본을 정할 뷰어 로케일이
없으므로) 영어 기준을 사용합니다. 배지 자체
