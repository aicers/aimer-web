...
<!-- Screenshot placeholder: 고객 설정의 "기본 분석 모델" 항목 — 현재
출처 줄, 모델 드롭다운, 저장 / 초기화 버튼. 실행 중인 스택에서 캡처
가능해지면 추가(이 화면은 aice-web-next 데이터에 의존하지 않음). -->
...
<!-- Screenshot placeholder: 관리자 설정의 "기본 분석 모델(전역)" 항목.
실행 중인 스택에서 캡처 가능해지면 추가(이 화면은 aice-web-next 데이터에
의존하지 않음). -->
