...
새 모델로 다시 큐에 넣는, 비용이 제한되고 운영자가 확인하는 실행 — 이
포함됩니다. 운영자 작업 흐름, 범위 지정, 비용 영향은
[스토리 리프 재분석 백필](reanalyze-backfill.md)을 참조하세요. 보고서
새로고침과 이벤트 리프 재분석은 별도로 제공되며 스토리 리프 실행이 모두
처리된 후 순차적으로 진행됩니다. 재분석을 의도적으로 시작하기 전까지
기존 결과는 그대로 유지됩니다.
