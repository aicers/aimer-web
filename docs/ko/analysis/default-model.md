...
재분석 페이지는 범위가 지정된 그 실행을 위한 안정적인 인앱 진입점입니다.
이제 이 페이지에는 **스토리 리프 재분석 백필** — 고객의 기존 스토리 분석을
새 모델로 다시 큐에 넣는, 비용이 제한되고 운영자가 확인하는 실행 — 과
**이벤트 리프 재분석 백필** — 자체 필수 비용 미리보기, 명시적 확인,
백그라운드 진행 상황/취소를 갖춘 실행 — 이 모두 포함됩니다. 각각
[스토리 리프 재분석 백필](reanalyze-backfill.md)과
[모델 변경 후 이벤트 리프 다시 분석](event-leaf-reanalysis.md)을
참조하세요. 두 리프 실행이 모두 처리되면 **보고서 변형 새로고침**이 고객의
정기 보고서를 재생성하여 재분석된 리프를 다시 집계합니다(
[모델 변경 후 정기 보고서 새로고침](report-variant-refresh.md) 참조).
재분석을 의도적으로 시작하기 전까지 기존 결과는 그대로 유지됩니다.
