...
- **위협 스토리** — 활성 범위 전반에서 우선순위가 가장 높은 위협
    스토리.
- **탐지 이벤트** — 활성 범위 전반에서 검토를 기다리는 탐지 이벤트.
...
탐색 항목 아래의 **분석 대상** 섹션에는 접근 가능한 분석 대상이
나열되며, 시각적으로 구분되는 두 하위 섹션으로 나뉩니다.

- **고객** — 접근 가능한 모든 고객. 각 항목은 해당 고객의
    [분석 허브](analysis/customer-hub.md)(`/subjects/<id>`)로 연결되며,
    허브는 고객의 보고서, 위협 스토리, 탐지 이벤트 요약을 한곳에 모아
    보여줍니다.
- **그룹** — 접근 가능한 모든 고객 그룹으로, 그룹 아이콘이 표시됩니다.
    각 항목은 해당 그룹의 [분석 허브](analysis/group-hub.md)로
    연결됩니다. 그룹은 **모든** 구성원에 대해 `reports:read` 권한을 가질
    때만 표시되며, 한 구성원이라도 접근할 수 없으면 목록에서 제외됩니다.

두 하위 섹션의 동작은 동일합니다.

- 이는 분석 대상 허브로 가는 **가장 빠른 경로**입니다. 상세 페이지로
    들어갔다가 브레드크럼을 거슬러 올라오지 않고 사이드바에서 바로
    엽니다.
- 분석 대상을 클릭하면 **해당 허브가 열립니다**. 고객 범위는 변경되지
    **않습니다** — 이 링크에는 `?scope=` 매개변수가 없으며 교차 고객
    뷰를 그대로 둡니다. (따라서 그룹 허브 링크는 아래 범위 선택기에서
    설명하는 그룹 **범위 프리셋**과 다릅니다. 링크는 그룹 허브를 열고,
    프리셋은 범위 필터를 채웁니다.)
- 이 섹션은 아래의 고객 범위 선택기와 **구분**됩니다. 범위 선택기는
    교차 고객 뷰를 위한 일시적 필터이고, 이 섹션은 각 분석 대상의 분석
    화면으로 가는 영구적인 링크 모음입니다.
- 현재 분석 대상의 허브 아래 어디에 있든 해당 링크가 강조되며,
    접근 가능한 대상이 많을 때 목록은 자체적으로 스크롤됩니다. 축소
    모드에서는 각 항목이 아이콘과 이름으로 표시되며, 마우스를 올리면
    툴팁이 나타납니다.

브릿지 세션에서는 이 섹션이 숨겨집니다. 브릿지 세션은 분석 대상의 분석
허브를 읽을 수 없고(브릿지에서는 허브가 `403`을 반환) 그룹 목록도
비어 있으므로, 나열할 수 있는 허브가 없습니다.

![사이드바 분석 대상 섹션(각각 분석 허브로 연결되는 고객 및 그룹 하위 섹션)에 대한 자리표시자 — 실데이터 재캡처 대기 중(그룹 하위 섹션은 접근 가능한 고객 그룹이 로드된 스택이 필요하며, 픽스처 스택으로는 재현할 수 없습니다)](../assets/navigation-summary-subjects.ko.png)
...
접근 가능한 고객 그룹이 하나 이상 있으면, 범위 선택기는 고객 체크박스
아래에 각 그룹을 **프리셋**으로 나열합니다. 프리셋을 선택하면 **그룹이
구성원 고객 ID로 확장되어** 위의 범위 필터에 적용됩니다 — 정확히 그룹
구성원 범위에서 교차 고객 뷰를 둘러보기에 편리합니다.

그룹 프리셋은 **순수한 뷰 필터**이며, (**분석 대상** 아래의) 그룹 허브를
여는 것과 의도적으로 구분됩니다.

- 프리셋은 그룹 구성원으로 **고객 다중 선택을 채울** 뿐, 아무 곳으로도
    이동하지 않고 어떤 산출물도 만들지 않습니다.
- 그룹 허브를 열면 그룹 자체의 분석 화면으로 **이동**하며 범위 필터를
    변경하지 않습니다.

브릿지 세션에서는 프리셋이 숨겨집니다(브릿지에서는 그룹이 나열되지
않습니다).
