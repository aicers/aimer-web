...
탐색 항목 아래의 **고객** 섹션에는 접근 가능한 모든 고객이
나열됩니다. 각 항목은 해당 고객의
[분석 허브](analysis/customer-hub.md)(`/subjects/<id>`)로 가는 직접
링크이며, 허브는 고객의 보고서, 위협 스토리, 의심 이벤트 요약을
한곳에 모아 보여줍니다.

- 이는 고객 허브로 가는 **가장 빠른 경로**입니다. 상세 페이지로
    들어갔다가 브레드크럼을 거슬러 올라오지 않고 사이드바에서 바로
    엽니다.
- 고객을 클릭하면 **해당 허브가 열립니다**. 고객 범위는 변경되지
    **않습니다** — 이 링크에는 `?scope=` 매개변수가 없으며 교차 고객
    뷰를 그대로 둡니다.
- 이 섹션은 아래의 고객 범위 선택기와 **구분**됩니다. 범위 선택기는
    교차 고객 뷰를 위한 일시적 필터이고, 이 섹션은 각 고객의 분석
    화면으로 가는 영구적인 링크 모음입니다.
- 현재 고객의 허브 아래 어디에 있든 해당 고객의 링크가 강조되며,
    접근 가능한 고객이 많을 때 목록은 자체적으로 스크롤됩니다. 축소
    모드에서는 각 항목이 아이콘과 고객 이름으로 표시되며, 마우스를
    올리면 툴팁이 나타납니다.

브릿지 세션에서는 이 섹션이 숨겨집니다. 브릿지 세션은 고객의 분석
허브를 읽을 수 없으므로(브릿지에서는 허브가 `403`을 반환), 나열할
수 있는 허브가 없습니다.
