...
지연 로드 요청이 실패하면 대화상자는 빈 배정 목록 대신 로드 오류를
표시하고 데이터가 로드될 때까지 **추가**를 비활성화합니다. 다시 시도하려면
대화상자를 다시 여세요. 이렇게 하면 일시적 장애가 분석가에게 배정이 없는
것처럼 보이거나 이미 배정된 고객이 추가 후보로 제시되는 것을 방지합니다.
...
대기 중 초대 목록을 불러오지 못하면 이 섹션은 "대기 중인 초대가 없습니다."
대신 로드 오류를 표시합니다. 로드 실패가 아직 취소해야 하는 초대를 숨겨서는
안 되기 때문입니다. 다시 시도하려면 페이지를 새로고침하세요.
